...
import { b2AABB, b2CircleShape, b2Manifold, b2ManifoldPoint, b2Mat22, b2Math, b2PolygonShape, b2Settings, b2Vec2, b2XForm, ClipVertex } from "..";
...
export class b2Collision{
											...
											poly2:b2PolygonShape, xf2:b2XForm):number
	{
		...
		var s:number = b2Collision.EdgeSeparation(poly1, xf1, edge, poly2, xf2);
		if (s > 0.0)
		{
			return s;
		}
		...
		var sPrev:number = b2Collision.EdgeSeparation(poly1, xf1, prevEdge, poly2, xf2);
		if (sPrev > 0.0)
		{
			return sPrev;
		}
		...
		var sNext:number = b2Collision.EdgeSeparation(poly1, xf1, nextEdge, poly2, xf2);
		if (sNext > 0.0)
		{
			return sNext;
		}
											...
											polyB:b2PolygonShape, xfB:b2XForm) : void
	{
		...
		var separationA:number = b2Collision.FindMaxSeparation(edgeAO, polyA, xfA, polyB, xfB);
		edgeA = edgeAO[0];
		if (separationA > 0.0)
			return;
		...
		var separationB:number = b2Collision.FindMaxSeparation(edgeBO, polyB, xfB, polyA, xfA);
		edgeB = edgeBO[0];
		if (separationB > 0.0)
			return;
