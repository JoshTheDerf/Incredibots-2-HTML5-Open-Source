...
import { Sprite, DisplayObject, Matrix } from "pixi.js";
...
import { SandboxSettings } from "./SandboxSettings";
	...
	{
		private static const LOGO_STRETCH_WIDTHS:Array<number> = [101.2, 101, 100.7, 100.5, 100.2, 100, 99.7, 99.4, 99.1, 98.9, 98.6, 98.4, 98.2, 98, 97.8, 97.7, 97.5, 97.5, 97.4, 97.4, 97.4, 97.5, 97.6, 97.8, 98.1, 98.3, 98.6, 99, 99.3, 99.6, 100, 100.3, 100.7, 101, 101.3, 101.6, 101.8, 102, 102.1, 102.2, 102.3, 102.2, 102.2, 102.1, 102, 101.8, 101.6, 101.4];
		private static const LOGO_STRETCH_HEIGHTS:Array<number> = [97.7, 97.6, 97.7, 97.7, 97.7, 97.8, 97.9, 98, 98.2, 98.4, 98.7, 99, 99.3, 99.6, 100, 100.3, 100.6, 100.9, 101.2, 101.5, 101.7, 101.8, 101.9, 102, 102, 101.9, 101.8, 101.7, 101.5, 101.3, 101.1, 100.8, 100.6, 100.3, 100.1, 99.8, 99.5, 99.3, 99, 98.8, 98.5, 98.4, 98.2, 98, 97.9, 97.8, 97.8, 97.7];

		static public ZOOM_FOCUS_X:number = 440;
		static public ZOOM_FOCUS_Y:number = 280;
		...
		private sSky:Sky;
		private sGround:Sprite;
		private sCanvas:Sprite;
		...
		private fader:Sprite;
		public fader2:Sprite;
		private logoFrame:number = 0;
		private versionText:TextField;
		private userText:TextField;
		...
		constructor(straightToLevelSelect:boolean = false)
		{
			...
			this.replay = Database.ExtractReplayFromByteArray(b);
			this.replay.cont = this;
			b = new Resource.cRobot();
			b.uncompress();
			...
			this.sGround = new Sprite();
			this.DrawGroundOutlineCircle(0, 0, 150);
			this.DrawGroundOutlineCircle(4000, 0, 150);
			m = new Matrix();
			m.createGradientBox(1, 300, Math.PI / 2, 0, 0);
			this.sGround.graphics.beginGradientFill(GradientType.LINEAR, [Util.HexColour(177, 102, 46), Util.HexColour(171, 59, 34)], [1, 1], [0, 255], m);
			this.sGround.graphics.drawRect(144, -6, 4012, 312);
			this.sGround.graphics.endFill();
			this.sGround.graphics.beginGradientFill(GradientType.LINEAR, [Util.HexColour(249, 172, 101), Util.HexColour(240, 70, 45)], [1, 1], [0, 255], m);
			...
			this.versionText = new TextField();
			this.versionText.x = 700;
			this.versionText.y = 580;
			this.versionText.width = 100;
			this.versionText.selectable = false;
			this.versionText.text = "Version " + Main.VERSION_STRING;
			this.versionText.setTextFormat(format);
			this.addChild(this.versionText);

			/*windowL = new Resource.cLevelSelectBox1L();
			windowL.x = 226;
			windowL.y = 300;
			addChild(windowL);
			windowR = new Resource.cLevelSelectBox1R();
			windowR.x = 550;
			windowR.y = 300;
			addChild(windowR);
			windowC = new Sprite();
			...
			m.translate(0, 300);
			windowC.graphics.beginBitmapFill(new Resource.cLevelSelectBox1M().bitmapData, m);
			windowC.graphics.drawRect(250, 300, 300, 135);
			addChild(windowC);
			format = new TextFormat();
			format.font = Main.GLOBAL_FONT;
			format.align = TextFormatAlign.CENTER;
			forumPostText = new TextField();
			forumPostText.x = 245;
			forumPostText.y = 310;
			forumPostText.width = 300;
			forumPostText.selectable = false;
			forumPostText.text = "The IncrediBots servers will be shut down soon.\nSaving has now been disabled, and instead\n\"Export\" and \"Import\" features are available.\nExport everything you want to keep so it won't be lost!";
			forumPostText.setTextFormat(format);
			addChild(forumPostText);
			format = new TextFormat();
			format.size = 16;
			forumPostButton = new GuiButton("More Info", 315, 365, 160, 55, forumPostButtonPressed, GuiButton.ORANGE, format);
			addChild(forumPostButton);*/

			this.fader = new Sprite();
			this.fader.graphics.beginFill(0, 0.2);
			this.fader.graphics.lineStyle(0, 0, 0.2);
			this.fader.graphics.moveTo(0, 0);
			this.fader.graphics.lineTo(800, 0);
			this.fader.graphics.lineTo(800, 600);
			this.fader.graphics.lineTo(0, 600);
			this.fader.graphics.lineTo(0, 0);
			this.fader.graphics.endFill();
			this.fader.visible = (straightToLevelSelect || Main.premiumMode);
			this.addChild(this.fader);

			this.sLogo = new Resource.cMainMenuLogo();
			...
			if (!straightToLevelSelect) {
				format = new TextFormat();
				format.size = 24;
				this.playButton = new GuiButton("Play!", 320, 184, 160, 70, this.playButtonPressed, GuiButton.PLAY, format);
			...
			var img:BitmapAsset = new Resource.cLevelSelectBox1L();
			img.x = 210;
			img.y = 105;
			this.levelSelectGui.addChild(img);
			img = new Resource.cLevelSelectBox1R();
			img.x = 576;
			img.y = 105;
			this.levelSelectGui.addChild(img);
			var m:Matrix = new Matrix();
			m.translate(0, 105);
			this.levelSelectGui.graphics.beginBitmapFill(new Resource.cLevelSelectBox1M().bitmapData, m);
			this.levelSelectGui.graphics.drawRect(234, 105, 342, 135);
			img = new Resource.cLevelSelectBox2L();
			img.x = 315;
			img.y = 260;
			this.levelSelectGui.addChild(img);
			img = new Resource.cLevelSelectBox2R();
			img.x = 472;
			img.y = 260;
			this.levelSelectGui.addChild(img);
			m = new Matrix();
			m.translate(0, 260);
			this.levelSelectGui.graphics.beginBitmapFill(new Resource.cLevelSelectBox2M().bitmapData, m);
			this.levelSelectGui.graphics.drawRect(339, 260, 133, 194);
			img = new Resource.cLevelSelectOtherBoxL();
			img.x = 617;
			img.y = 449;
			this.levelSelectGui.addChild(img);
			img = new Resource.cLevelSelectOtherBoxR();
			img.x = 763;
			img.y = 449;
			this.levelSelectGui.addChild(img);
			m = new Matrix();
			m.translate(0, 449);
			this.levelSelectGui.graphics.beginBitmapFill(new Resource.cLevelSelectOtherBoxM().bitmapData, m);
			this.levelSelectGui.graphics.drawRect(641, 449, 122, 144);

			format = new TextFormat();
			format.size = 17;
			var button:GuiButton = new GuiButton("Tutorial Levels", 210, 109, 200, 65, this.tutorialButton, GuiButton.PINK, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Sandbox Mode", 390, 109, 200, 65, this.sandboxButton, GuiButton.PINK, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Challenge Editor", 210, 158, 200, 65, this.editorButton, GuiButton.PINK, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton(" Advanced Sandbox ", 390, 158, 200, 65, this.advancedButton, GuiButton.PINK, format);
			this.levelSelectGui.addChild(button);
			this.logInButton = new GuiButton("Log In", 675, -5, 120, 60, this.loginButton, GuiButton.BLUE, format);
			this.logInButton.visible = (ControllerGame.userName == "_Public");
			this.levelSelectGui.addChild(this.logInButton);
			this.logOutButton = new GuiButton("Log Out", 675, -5, 120, 60, this.logout, GuiButton.BLUE, format);
			this.logOutButton.visible = (ControllerGame.userName != "_Public");
			this.levelSelectGui.addChild(this.logOutButton);
			this.enableSoundButton = new GuiButton("Enable Sound", 5, 535, 150, 60, this.enableSoundButtonPressed, GuiButton.BLUE, format);
			this.enableSoundButton.visible = !Main.enableSound;
			this.disableSoundButton = new GuiButton("Disable Sound", 5, 535, 150, 60, this.disableSoundButtonPressed, GuiButton.BLUE, format);
			...
			button = new GuiButton("Load Challenge", 315, 263, 171, 59, this.loadChallengeButton, GuiButton.BLUE, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Load Replay", 315, 303, 171, 59, this.loadReplayButton, GuiButton.BLUE, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Load Bot", 315, 343, 171, 59, this.loadRobotButton, GuiButton.BLUE, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("High Scores", 315, 383, 171, 59, this.highScoresButton, GuiButton.RED, format);
			this.levelSelectGui.addChild(button);

			button = new GuiButton("Import Challenge", 145, 263, 171, 59, this.importChallengeButton, GuiButton.ORANGE, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Import Replay", 145, 303, 171, 59, this.importReplayButton, GuiButton.ORANGE, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Import Bot", 145, 343, 171, 59, this.importRobotButton, GuiButton.ORANGE, format);
			this.levelSelectGui.addChild(button);

			button = new GuiButton("Instructions", 620, 454, 153, 50, this.instructionsButton, GuiButton.BLUE, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Credits", 620, 489, 153, 50, this.creditsButton, GuiButton.BLUE, format);
			this.levelSelectGui.addChild(button);
			button = new GuiButton("Suggestions?", 620, 524, 153, 50, this.suggestionsButton, GuiButton.BLUE, format);
			this.levelSelectGui.addChild(button);
			...
			this.userText.selectable = false;
			...
			if (!LSOManager.IsAnythingDone()) {
				this.startHereArrow = new Resource.cLevelSelectStartHereArrow();
				...
				this.arrowX = 130;
				this.startHereText = new Resource.cLevelSelectStartHereText();
			...
			this.fader2 = new Sprite();
			this.fader2.graphics.beginFill(0, 0.2);
			this.fader2.graphics.lineStyle(0, 0, 0.2);
			this.fader2.graphics.moveTo(0, 0);
			this.fader2.graphics.lineTo(800, 0);
			this.fader2.graphics.lineTo(800, 600);
			this.fader2.graphics.lineTo(0, 600);
			this.fader2.graphics.lineTo(0, 0);
			this.fader2.graphics.endFill();
		...
		private DrawRock(type:number, xPos:number, yPos:number, radius:number, outlineThickness:number = 6):void {
			this.sGround.graphics.lineStyle(outlineThickness, 0xBA643D);
			var m:Matrix = new Matrix();
			m.createGradientBox(radius * 2, radius * 2, Math.PI / 2, xPos, yPos);
			this.sGround.graphics.beginGradientFill(GradientType.LINEAR, (type == 0 ? [Util.HexColour(194, 130, 87), Util.HexColour(172,	114,	77)] : (type == 1 ? [Util.HexColour(197,	115,	66), Util.HexColour(175,	103,	58)] : [Util.HexColour(198,	121,	69), Util.HexColour(179,	105,	57)])), [1, 1], [0, 255], m);
			this.sGround.graphics.drawCircle(xPos + radius, yPos + radius, radius);
			this.sGround.graphics.endFill();
		}
		...
		public Update():void {
			...
			if (this.replay.Update(this.frameCounter)) {
				this.frameCounter = 0;
