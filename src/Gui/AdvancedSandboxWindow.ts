...
export class AdvancedSandboxWindow extends GuiWindow
{
	...
	constructor(contr:Controller, defaults:SandboxSettings|null = null)
	{
		...
		format = new TextStyle();
		format.fontSize = 10;
		this.gravityArea = new GuiTextInput(104, 316, 30, 20, format);
		this.gravityArea.text = "15";
		if (defaults) this.gravityArea.text = (defaults.gravity as int) + "";
		...
		format = new TextStyle();
		format.fontSize = 13;
		this.cancelButton = new GuiButton("Cancel", 74, 385, 100, 35, this.cancelButtonPressed.bind(this), GuiButton.PURPLE, format);
	...
	private okButtonPressed(e:MouseEvent):void {
		var settings:SandboxSettings = new SandboxSettings(this.gravitySlider.value, this.sizeBox.selectedIndex, this.shapeBox.selectedIndex, this.themeBox.selectedIndex, this.bgBox.selectedIndex, parseInt(this.redArea.text), parseInt(this.greenArea.text), parseInt(this.blueArea.text));
