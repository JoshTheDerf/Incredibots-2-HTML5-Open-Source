...
import { b2Body, b2Collision, b2Contact, b2ContactListener, b2ContactPoint, b2Manifold, b2ManifoldPoint, b2Mat22, b2PolygonShape, b2Settings, b2Shape, b2StaticEdgeShape, b2Vec2, b2XForm } from "../..";
...
export class b2PolyAndStaticEdgeContact extends b2Contact{
	...
	public Evaluate(listener:b2ContactListener): void{
		...
		if (this.m_manifold.pointCount > 0)
		{
			...
			var pc:number = this.m_manifold.pointCount;
			for (i = 0; i < pc; ++i)
			{
				...
				var pc2:number = this.m0.pointCount;
				for (var j:number = 0; j <pc2; ++j)
				{
					if (persisted[j] == true)
					{
						continue;
