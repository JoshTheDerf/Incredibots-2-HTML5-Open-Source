...
export class b2DebugDraw {
    ...
    if (this.IsPolygonOnScreen(vertices, vertexCount)) {
      ...
      for (var i: number = 1; i < vertexCount; i++) {
        this.m_sprite.lineTo(
          vertices[i].x * this.m_drawScale - this.m_drawXOff,
          vertices[i].y * this.m_drawScale - this.m_drawYOff
        );
      }
      this.m_sprite.lineTo(
        vertices[0].x * this.m_drawScale - this.m_drawXOff,
  ...
  public DrawSolidCannon(
    ...
    if (this.IsPolygonOnScreen(vertices, vertexCount)) {
      ...
      this.m_sprite.lineTo(
        vertices[2].x * this.m_drawScale - this.m_drawXOff,
        vertices[2].y * this.m_drawScale - this.m_drawYOff
      );
      this.m_sprite.lineTo(
      ...
      this.m_sprite.lineTo(
        (vertices[1].x + vertices[2].x) * 0.5 * this.m_drawScale - this.m_drawXOff,
        (vertices[1].y + vertices[2].y) * 0.5 * this.m_drawScale - this.m_drawYOff
      );
      this.m_sprite.lineTo(
        vertices[3].x * this.m_drawScale - this.m_drawXOff,
        vertices[3].y * this.m_drawScale - this.m_drawYOff
    ...
    if (this.IsCircleOnScreen(center, radius)) {
      ...
      if (drawOutlines && !cannonball) {
        ...
        for (var i: number = 0; i < numSpokes; i++) {
          ...
          this.m_sprite.lineTo(
            (center.x + Math.cos(angle) * radius) * this.m_drawScale - this.m_drawXOff,
  ...
  public IsCircleOnScreen(center, radius: number): boolean {
    ...
    return (
      screenCenter.x + newRadius >= -5 &&
      screenCenter.x - newRadius < 805 &&
      screenCenter.y + newRadius >= -5 &&
      screenCenter.y - newRadius < 605
    );
  }
