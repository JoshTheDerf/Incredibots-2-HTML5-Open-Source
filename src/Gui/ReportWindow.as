	...
	public class ReportWindow extends GuiWindow
	{
		...
		private function reportClicked(e:Event):void {
			...
			if (parentWindow is SaveLoadWindow) {
				...
				if (dataType == 0) {
					Database.ReportRobot(id, ControllerGameGlobals.userName, descArea.text, p.finishReporting);
				} else if (dataType == 1) {
					Database.ReportReplay(id, ControllerGameGlobals.userName, descArea.text, p.finishReporting);
				} else {
					Database.ReportChallenge(id, ControllerGameGlobals.userName, descArea.text, p.finishReporting);
				}
			} else {
				var pa:ControllerGame = (parentWindow as ControllerGame);
				pa.ShowDialog("Reporting...");
				if (dataType == 0) {
					Database.ReportRobot(id, ControllerGameGlobals.userName, descArea.text, pa.finishReporting);
				} else if (dataType == 1) {
					Database.ReportReplay(id, ControllerGameGlobals.userName, descArea.text, pa.finishReporting);
				} else {
					Database.ReportChallenge(id, ControllerGameGlobals.userName, descArea.text, pa.finishReporting);
				}
			}
		}
