...
export class ControllerGame extends Controller {
    ...
    if (ControllerGameGlobals.playingReplay) {
      ...
      for (var i: number = 0; i < this.allParts.length; i++) {
        if (this.allParts[i] instanceof TextPart) {
          var newTextPart: TextPart = new TextPart(
            this,
            this.allParts[i].x,
            this.allParts[i].y,
            this.allParts[i].w,
            this.allParts[i].h,
            this.allParts[i].text
          );
  ...
  public PlayShapeSound(): void {
    if (Main.enableSound) {
      ...
      var sound: Sound =
        soundNum == 0
          ? ControllerGameGlobals.shapeSound1
          : soundNum == 1
          ? ControllerGameGlobals.shapeSound2
          : soundNum == 2
          ? ControllerGameGlobals.shapeSound3
          : soundNum == 3
          ? ControllerGameGlobals.shapeSound4
          : ControllerGameGlobals.shapeSound5;
      sound.play();
    }
  }
  ...
  public PlayJointSound(): void {
    if (Main.enableSound) {
      ...
      var sound: Sound =
        soundNum == 0
          ? ControllerGameGlobals.jointSound1
          : soundNum == 1
          ? ControllerGameGlobals.jointSound2
          : soundNum == 2
          ? ControllerGameGlobals.jointSound3
          : soundNum == 3
          ? ControllerGameGlobals.jointSound4
          : ControllerGameGlobals.jointSound5;
      sound.play();
    }
  }
  ...
  public Update(): void {
    ...
    if (!this.m_fader.visible) {
      ...
      for (var i: number = 0; i < this.allParts.length; i++) {
        if (this.allParts[i] instanceof RevoluteJoint || this.allParts[i] instanceof PrismaticJoint)
          this.allParts[i].CheckForBreakage(this.m_world);
      }

      this.m_guiMenu.Update();
      this.m_guiPanel.Update(this.curAction);

      this.uneditableText.visible =
        !ControllerGameGlobals.curRobotEditable && !ControllerGameGlobals.playingReplay && !this.simStarted;
    ...
    if (!this.simStarted) {
      for (i = 0; i < this.m_buildAreas.length; i++) {
        ...
        if (this.hasZoomed || this.redrawBuildArea) {
          ...
          this.m_selectedBuildAreas[i].width =
            this.World2ScreenX(box.upperBound.x) - this.World2ScreenX(box.lowerBound.x);
          this.m_selectedBuildAreas[i].height =
            this.World2ScreenY(box.upperBound.y) - this.World2ScreenY(box.lowerBound.y);
        }
    ...
    if (!this.simStarted) {
      if (
        ...
        this.candidateJointType == ControllerGameGlobals.NEW_PRISMATIC_JOINT &&
        this.actionStep == 1
      ) {
        this.draw.DrawTempShape(
          ControllerGameGlobals.NEW_PRISMATIC_JOINT,
          this.actionStep,
          this.firstClickX,
          this.firstClickY,
          this.secondClickX,
          this.secondClickY,
          this.candidateJointX,
          this.candidateJointY
        );
      } else if (
        this.curAction == ControllerGameGlobals.FINALIZING_JOINT &&
        this.candidateJointType == ControllerGameGlobals.NEW_THRUSTERS
      ) {
        this.draw.DrawTempShape(
          ControllerGameGlobals.NEW_THRUSTERS,
          this.actionStep,
          this.firstClickX,
          this.firstClickY,
          this.secondClickX,
        ...
        this.draw.DrawTempShape(
          ...
          ControllerGameGlobals.mouseYWorldPhys
        );
      }
    }
  ...
  public AddSyncPoint(): void {
    if (this.syncPoints.length == 0 || this.frameCounter != this.syncPoints[this.syncPoints.length - 1].frame) {
      ...
      for (var i: number = 0; i < this.allParts.length; i++) {
        if (
          this.allParts[i] instanceof ShapePart &&
          !this.allParts[i].isStatic &&
          !Util.ObjectInArray(this.allParts[i].GetBody(), bodiesUsed)
        ) {
          syncPoint.positions.push(
            Util.Vector(this.allParts[i].GetBody().GetPosition().x, this.allParts[i].GetBody().GetPosition().y)
          );
      ...
      for (i = 0; i < ControllerGameGlobals.cannonballs.length; i++) {
        syncPoint.cannonballPositions.push(
          Util.Vector(
            ControllerGameGlobals.cannonballs[i].GetPosition().x,
            ControllerGameGlobals.cannonballs[i].GetPosition().y
          )
        );
      }
  ...
  private ComputeReplaySplines(type: number): Array<any> {
    ...
    for (var i: number = 0; i < ControllerGameGlobals.replay.syncPoints.length - 1; i++) {
      ...
      for (var j: number = 0; j < ControllerGameGlobals.replay.syncPoints[0].positions.length; j++) {
        if (type == 0)
          inner.push(
            (ControllerGameGlobals.replay.syncPoints[i + 1].positions[j].x -
              ControllerGameGlobals.replay.syncPoints[i].positions[j].x) /
              h[i]
          );
        else if (type == 1)
          inner.push(
            (ControllerGameGlobals.replay.syncPoints[i + 1].positions[j].y -
              ControllerGameGlobals.replay.syncPoints[i].positions[j].y) /
              h[i]
          );
        else {
          var deltaAngle: number =
            ControllerGameGlobals.replay.syncPoints[i + 1].angles[j] -
            ControllerGameGlobals.replay.syncPoints[i].angles[j];
      ...
      for (j = 0; j < ControllerGameGlobals.replay.syncPoints[0].positions.length; j++) {
        ...
        innerB.push(
          (-h[i] * z[i + 1][j]) / 6 -
            (h[i] * z[i][j]) / 3 +
            (type == 0
              ? ControllerGameGlobals.replay.syncPoints[i + 1].positions[j].x - innerA[j]
              : type == 1
              ? ControllerGameGlobals.replay.syncPoints[i + 1].positions[j].y - innerA[j]
              : deltaAngle) /
              h[i]
        );
        innerC.push(z[i][j] / 2);
  ...
  public SyncReplay2(syncPoint1: Object, syncPoint2: Object): void {
    ...
    for (i = 0; i < ControllerGameGlobals.cannonballs.length; i++) {
      if (syncPoint1.cannonballPositions.length > i) {
        ...
        var newX: number =
          (syncPoint1.cannonballPositions[i].x * (syncPoint2.frame - this.frameCounter) +
            syncPoint2.cannonballPositions[i].x * (this.frameCounter - syncPoint1.frame)) /
          frameDiff;
        var newY: number =
    ...
    for (var i: number = 0; i < partsToCheck.length; i++) {
      ...
      for (var j: number = 0; j < numAreas; j++) {
        ...
        if (partsToCheck[i] instanceof Circle) {
          ...
          partsToCheck[i] instanceof Rectangle ||
          partsToCheck[i] instanceof Triangle ||
          partsToCheck[i] instanceof Cannon
        ) {
        ...
        if (
          minX >= buildingArea.lowerBound.x &&
          minY >= buildingArea.lowerBound.y &&
          maxX < buildingArea.upperBound.x &&
          maxY < buildingArea.upperBound.y
        ) {
          partFits = true;
          break;
        }
  ...
  public MouseDrag(): void {
    if (!this.simStarted) {
      ...
      if (
        Input.mouseDown &&
        ControllerGameGlobals.mouseYWorld >= 100 &&
        !this.m_sidePanel.sliderDown &&
        !this.m_guiMenu.MouseOverMenu(ControllerGameGlobals.mouseXWorld, ControllerGameGlobals.mouseYWorld) &&
        (ControllerGameGlobals.mouseXWorld >= 120 || !this.m_sidePanel.visible) &&
        (ControllerGameGlobals.mouseXWorld >= 230 ||
          ControllerGameGlobals.mouseYWorld >= 600 ||
          !this.m_sidePanel.ColourWindowShowing()) &&
        ...
        if (
          !this.m_tutorialDialog ||
          !this.m_tutorialDialog.MouseOver(ControllerGameGlobals.mouseXWorld, ControllerGameGlobals.mouseYWorld)
        ) {
          var shapeOrJoint: Part = this.GetPartAtMouse();

          if (
            !ControllerGameGlobals.wasMouseDown &&
            ControllerGameGlobals.curRobotEditable &&
            shapeOrJoint instanceof ShapePart
          ) {
            var part: ShapePart = shapeOrJoint as ShapePart;
            // rotation
            if (
              this.draggingPart == null &&
              (this.curAction == -1 || this.curAction == ControllerGameGlobals.SELECTING_SHAPE)
            ) {
              ...
              for (var i: number = 0; i < this.selectedParts.length; i++) {
                this.draggingParts = Util.RemoveDuplicates(
                  this.draggingParts.concat(this.selectedParts[i].GetAttachedParts())
                );
              }
              for (i = 0; i < this.draggingParts.length; i++) {
                if (this.draggingParts[i] instanceof ShapePart || this.draggingParts[i] instanceof Thrusters) {
                  this.draggingParts[i].dragXOff =
                    ControllerGameGlobals.mouseXWorldPhys - this.draggingParts[i].centerX;
                  this.draggingParts[i].dragYOff =
                    ControllerGameGlobals.mouseYWorldPhys - this.draggingParts[i].centerY;
                } else if (this.draggingParts[i] instanceof JointPart) {
                  this.draggingParts[i].dragXOff =
                    ControllerGameGlobals.mouseXWorldPhys - this.draggingParts[i].anchorX;
                  this.draggingParts[i].dragYOff =
                    ControllerGameGlobals.mouseYWorldPhys - this.draggingParts[i].anchorY;
                } else {
            ...
            !ControllerGameGlobals.wasMouseDown &&
            ControllerGameGlobals.curRobotEditable &&
            shapeOrJoint instanceof JointPart &&
            this.draggingPart == null &&
            (this.curAction == -1 || this.curAction == ControllerGameGlobals.SELECTING_SHAPE)
          ) {
            if (this.MouseOverSelectedPart()) {
            ...
            !ControllerGameGlobals.wasMouseDown &&
            ControllerGameGlobals.curRobotEditable &&
            shapeOrJoint instanceof TextPart &&
            this.draggingPart == null &&
            (this.curAction == -1 || this.curAction == ControllerGameGlobals.SELECTING_SHAPE)
          ) {
            if (this.MouseOverSelectedPart()) {
            ...
            this.initDragY = ControllerGameGlobals.mouseYWorldPhys;
            if (
              (shapeOrJoint as TextPart).InsideMoveBox(
                ControllerGameGlobals.mouseXWorldPhys,
                ControllerGameGlobals.mouseYWorldPhys,
                this.m_physScale,
                true
            ...
            !ControllerGameGlobals.wasMouseDown &&
            ControllerGameGlobals.curRobotEditable &&
            shapeOrJoint instanceof Thrusters &&
            this.draggingPart == null &&
            (this.curAction == -1 || this.curAction == ControllerGameGlobals.SELECTING_SHAPE)
          ) {
            if (this.MouseOverSelectedPart()) {
      ...
      if (this.draggingPart != null && this.curAction != ControllerGameGlobals.RESIZING_TEXT) {
        for (i = 0; i < this.draggingParts.length; i++) {
          this.draggingParts[i].Move(
            ControllerGameGlobals.mouseXWorldPhys - this.draggingParts[i].dragXOff,
            ControllerGameGlobals.mouseYWorldPhys - this.draggingParts[i].dragYOff
          );
        }
        ControllerGameGlobals.curRobotID = "";
      }
      ...
      if (this.curAction == ControllerGameGlobals.ROTATE && this.rotatingPart != null) {
        var newAngle: number = Math.atan2(
          ControllerGameGlobals.mouseYWorldPhys - this.rotatingPart.centerY,
          ControllerGameGlobals.mouseXWorldPhys - this.rotatingPart.centerX
        );
        for (i = 0; i < this.rotatingParts.length; i++) {
          this.rotatingParts[i].RotateAround(
            this.rotatingPart.centerX,
            this.rotatingPart.centerY,
            newAngle - this.initRotatingAngle
          );
        }
        ControllerGameGlobals.curRobotID = "";
      }
      ...
      if (
        ...
        this.selectedParts[0] instanceof ShapePart &&
        this.selectedParts[0] != this.m_conditionsDialog.shape1
      ) {
      ...
      if (this.curAction == ControllerGameGlobals.RESIZING_SHAPES) {
        ...
        for (i = 0; i < this.draggingParts.length; i++) {
          if (this.draggingParts[i] instanceof Circle) {
            if (this.draggingParts[i].initRadius * scaleFactor > Circle.MAX_RADIUS)
              scaleFactor = Circle.MAX_RADIUS / this.draggingParts[i].initRadius;
            if (this.draggingParts[i].initRadius * scaleFactor < Circle.MIN_RADIUS)
              scaleFactor = Circle.MIN_RADIUS / this.draggingParts[i].initRadius;
          } else if (this.draggingParts[i] instanceof Rectangle) {
            if (this.draggingParts[i].initW * scaleFactor > Rectangle.MAX_WIDTH)
              scaleFactor = Rectangle.MAX_WIDTH / this.draggingParts[i].initW;
            if (this.draggingParts[i].initW * scaleFactor < Rectangle.MIN_WIDTH)
              scaleFactor = Rectangle.MIN_WIDTH / this.draggingParts[i].initW;
            if (this.draggingParts[i].initH * scaleFactor > Rectangle.MAX_WIDTH)
              scaleFactor = Rectangle.MAX_WIDTH / this.draggingParts[i].initH;
            if (this.draggingParts[i].initH * scaleFactor < Rectangle.MIN_WIDTH)
              scaleFactor = Rectangle.MIN_WIDTH / this.draggingParts[i].initH;
          } else if (this.draggingParts[i] instanceof Triangle) {
            var length1: number = Util.GetDist(
              this.draggingParts[i].initX1,
              this.draggingParts[i].initY1,
              this.draggingParts[i].initX2,
              this.draggingParts[i].initY2
            );
            ...
            var length2: number = Util.GetDist(
              this.draggingParts[i].initX1,
              this.draggingParts[i].initY1,
              this.draggingParts[i].initX3,
              this.draggingParts[i].initY3
            );
            ...
            var length3: number = Util.GetDist(
              this.draggingParts[i].initX2,
              this.draggingParts[i].initY2,
              this.draggingParts[i].initX3,
              this.draggingParts[i].initY3
            ...
            if (this.draggingParts[i].initW * scaleFactor > Cannon.MAX_WIDTH)
              scaleFactor = Cannon.MAX_WIDTH / this.draggingParts[i].initW;
            if (this.draggingParts[i].initW * scaleFactor < Cannon.MIN_WIDTH)
              scaleFactor = Cannon.MIN_WIDTH / this.draggingParts[i].initW;
          }
        }
        ...
        for (i = 0; i < this.draggingParts.length; i++) {
          if (this.draggingParts[i] instanceof Circle) {
            ...
            this.draggingParts[i].Move(
              this.initDragX + this.draggingParts[i].dragXOff * scaleFactor,
              this.initDragY + this.draggingParts[i].dragYOff * scaleFactor
            );
          } else if (this.draggingParts[i] instanceof Rectangle) {
            ...
            this.draggingParts[i].w = this.draggingParts[i].initW * scaleFactor;
            this.draggingParts[i].h = this.draggingParts[i].initH * scaleFactor;
            this.draggingParts[i].x =
              this.initDragX + this.draggingParts[i].dragXOff * scaleFactor - this.draggingParts[i].w / 2;
            this.draggingParts[i].y =
              this.initDragY + this.draggingParts[i].dragYOff * scaleFactor - this.draggingParts[i].h / 2;
      ...
      if (
        ...
        if (this.candidateJointType == ControllerGameGlobals.NEW_REVOLUTE_JOINT) {
          ...
          var fjoint: JointPart = new FixedJoint(
            this.potentialJointPart1,
            this.potentialJointPart2,
            this.candidateJointX,
            this.candidateJointY
          ...
          var pjoint: PrismaticJoint = new PrismaticJoint(
            this.jointPart,
            this.potentialJointPart1,
            this.firstClickX,
            this.firstClickY,
      ...
      if (this.curAction == ControllerGameGlobals.RESIZING_TEXT) {
        (this.draggingPart as TextPart).Resize(
          ControllerGameGlobals.mouseXWorldPhys - this.initDragX,
          ControllerGameGlobals.mouseYWorldPhys - this.initDragY
        );
        this.redrawRobot = true;
      }
    } else if (
    ...
    if (
      ...
      if (
        (!this.m_tutorialDialog ||
          !this.m_tutorialDialog.MouseOver(ControllerGameGlobals.mouseXWorld, ControllerGameGlobals.mouseYWorld)) &&
        !this.draggingTutorial
      ) {
        // dragging the world around
        var change: boolean =
          ControllerGameGlobals.mouseXWorld != ControllerGameGlobals.prevMouseXWorld ||
          ControllerGameGlobals.mouseYWorld != ControllerGameGlobals.prevMouseYWorld;
        this.draw.m_drawXOff -= ControllerGameGlobals.mouseXWorld - ControllerGameGlobals.prevMouseXWorld;
        this.draw.m_drawYOff -= ControllerGameGlobals.mouseYWorld - ControllerGameGlobals.prevMouseYWorld;
        this.autoPanning = false;
        ...
        if (change) {
          this.hasPanned = true;
          this.cameraMovements.push(
            new CameraMovement(this.frameCounter, this.draw.m_drawXOff, this.draw.m_drawYOff, this.m_physScale)
          );
        }
      }
    }
  ...
  public keyInput(key: number, up: boolean): void {
    ...
    for (var i: number = 0; i < this.allParts.length; i++) {
      ...
      if (
        ...
        ((this.allParts[i] instanceof TextPart && key == this.allParts[i].displayKey) ||
          (this.allParts[i] instanceof Cannon && key == this.allParts[i].fireKey))
      ) {
        recorded = true;
  ...
  public mouseClick(up: boolean): void {
    ...
    if (up && this.rotatingPart != null) {
      ...
      this.AddAction(
        new RotateAction(
          this.rotatingPart as Part,
          this.rotatingParts,
          Util.NormalizeAngle(
            Math.atan2(
              ControllerGameGlobals.mouseYWorldPhys - this.rotatingPart.centerY,
              ControllerGameGlobals.mouseXWorldPhys - this.rotatingPart.centerX
            ) - this.initRotatingAngle
          )
        )
      );
      ...
      if (
        Math.abs(this.firstClickX - ControllerGameGlobals.mouseXWorldPhys) > 0.1 ||
        Math.abs(this.firstClickY - ControllerGameGlobals.mouseYWorldPhys) > 0.1
      ) {
        this.curAction = -1;
        this.m_conditionsDialog.visible = true;
        this.m_fader.visible = true;
        this.selectingCondition = false;
        this.m_conditionsDialog.FinishDrawingCondition(
          this.firstClickX,
          this.firstClickY,
          ControllerGameGlobals.mouseXWorldPhys,
          ControllerGameGlobals.mouseYWorldPhys
      ...
      if (
        Math.abs(this.firstClickX - ControllerGameGlobals.mouseXWorldPhys) > 0.5 ||
        ...
        buildArea.upperBound = Util.Vector(
    ...
    if (
      ...
      this.m_guiMenu.MouseOverMenu(ControllerGameGlobals.mouseXWorld, ControllerGameGlobals.mouseYWorld) ||
      (ControllerGameGlobals.mouseXWorld < 230 &&
        ControllerGameGlobals.mouseYWorld < 340 &&
        this.m_sidePanel.ColourWindowShowing()) ||
      (this.m_tutorialDialog &&
        this.m_tutorialDialog.MouseOver(ControllerGameGlobals.mouseXWorld, ControllerGameGlobals.mouseYWorld))
    )
    ...
    if (this.curAction == ControllerGameGlobals.NEW_CIRCLE) {
      if (!up && this.actionStep == 0) {
        ...
        var radius: number = Util.GetDist(
          this.firstClickX,
          this.firstClickY,
          ControllerGameGlobals.mouseXWorldPhys,
          ControllerGameGlobals.mouseYWorldPhys
      ...
      if (!up && this.actionStep == 0) {
        ...
        if (
          ControllerGameGlobals.mouseXWorldPhys != this.firstClickX ||
          ControllerGameGlobals.mouseYWorldPhys != this.firstClickY
        ) {
          this.curAction = -1;
          var rect: ShapePart = new Rectangle(
            this.firstClickX,
            this.firstClickY,
            ControllerGameGlobals.mouseXWorldPhys - this.firstClickX,
            ControllerGameGlobals.mouseYWorldPhys - this.firstClickY
      ...
      if (!up && this.actionStep == 0) {
        ...
        if (
          ...
          var positive: boolean =
            ControllerGameGlobals.mouseXWorldPhys >= this.firstClickX ||
            ControllerGameGlobals.mouseYWorldPhys >= this.firstClickY;
          var w: number = positive
            ? Math.max(
                ControllerGameGlobals.mouseXWorldPhys - this.firstClickX,
                2 * (ControllerGameGlobals.mouseYWorldPhys - this.firstClickY)
              )
            : Math.min(
                ControllerGameGlobals.mouseXWorldPhys - this.firstClickX,
                2 * (ControllerGameGlobals.mouseYWorldPhys - this.firstClickY)
      ...
      if (!up && this.actionStep == 0) {
        ...
        if (
          ...
          var sideLen: number = Util.GetDist(
            this.firstClickX,
            this.firstClickY,
            ControllerGameGlobals.mouseXWorldPhys,
            ControllerGameGlobals.mouseYWorldPhys
        ...
        if (
          (ControllerGameGlobals.mouseXWorldPhys != this.firstClickX ||
            ControllerGameGlobals.mouseYWorldPhys != this.firstClickY) &&
          (ControllerGameGlobals.mouseXWorldPhys != this.secondClickX ||
            ControllerGameGlobals.mouseYWorldPhys != this.secondClickY)
        ) {
          var sideLen1: number = Util.GetDist(
            this.firstClickX,
            this.firstClickY,
            ControllerGameGlobals.mouseXWorldPhys,
            ControllerGameGlobals.mouseYWorldPhys
          );
          var sideLen2: number = Util.GetDist(
            this.secondClickX,
            this.secondClickY,
            ControllerGameGlobals.mouseXWorldPhys,
            ControllerGameGlobals.mouseYWorldPhys
          ...
          if (
            ...
            this.curAction = -1;
            var tri: ShapePart = new Triangle(
              this.firstClickX,
              this.firstClickY,
              this.secondClickX,
              this.secondClickY,
      ...
      if (
        ...
        this.MaybeFinishCreatingPrismaticJoint();
    } else if (
      up &&
      this.actionStep == 0 &&
      (this.curAction == ControllerGameGlobals.NEW_REVOLUTE_JOINT ||
      ...
      for (i = 0; i < this.allParts.length; i++) {
        if (
          this.allParts[i].isEditable &&
          this.allParts[i].IntersectsBox(
            Math.min(this.firstClickX, ControllerGameGlobals.mouseXWorldPhys),
            Math.min(this.firstClickY, ControllerGameGlobals.mouseYWorldPhys),
      ...
      var text: TextPart = new TextPart(
        this,
        Math.min(this.firstClickX, finalClickX),
        Math.min(this.firstClickY, finalClickY),
        Math.max(this.firstClickX, finalClickX) - Math.min(this.firstClickX, finalClickX),
        Math.max(this.firstClickY, finalClickY) - Math.min(this.firstClickY, finalClickY),
        "New Text"
      );
  ...
  public thrustersButton(): void {
    if (/*Main.premiumMode*/ true) {
      ...
      this.ShowConfirmDialog(
        "That feature instanceof only available to IncrediBots supporters. Become an IncrediBots supporter?",
        8
      );
    }
  }
    ...
    if (!ControllerGameGlobals.adStarted) {
      ...
      if ((this.partsFit && !tooManyShapes) || ControllerGameGlobals.playingReplay) {
        ...
        if (!this.simStarted) {
          ...
          this.cameraMovements.push(
            new CameraMovement(0, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, this.m_physScale)
          );
          ...
          for (i = this.allParts.length; i >= 0; i--) {
            if (this.allParts[i] instanceof ShapePart || this.allParts[i] instanceof TextPart)
          ...
          for (i = 0; i < this.allParts.length; i++) {
            ...
            if (this.allParts[i] instanceof JointPart || this.allParts[i] instanceof Thrusters)
              this.allParts[i].Init(this.m_world);
          }
  ...
  public pauseButton(): void {
    if (ControllerGameGlobals.playingReplay && ControllerGameGlobals.replay.Update(this.frameCounter))
      this.ShowPostReplayWindow();
    ...
    if (ControllerGameGlobals.playingReplay) {
      ...
      if (rateRobot) {
        if (
          ...
          !ControllerGameGlobals.ratedCurChallenge &&
          !LSOManager.HasRatedChallenge(ControllerGameGlobals.curChallengeID)
        ) {
          ...
          ControllerGameGlobals.curRobotID != "" &&
          ControllerGameGlobals.curRobotPublic &&
          !ControllerGameGlobals.ratedCurRobot &&
          !LSOManager.HasRatedRobot(ControllerGameGlobals.curRobotID)
        ) {
  ...
  public rewindButton(makeThemRate: boolean = true): void {
    if (!ControllerGameGlobals.playingReplay) {
      ...
      if (
        ...
        !ControllerGameGlobals.ratedCurReplay &&
        ControllerGameGlobals.curReplayPublic &&
        !LSOManager.HasRatedReplay(ControllerGameGlobals.curReplayID)
      ) {
  ...
  public featureButton(): void {
    ...
    if (
      ...
      Database.FeatureReplay(
        ControllerGameGlobals.curReplayID,
        !ControllerGameGlobals.curReplayFeatured,
        this.finishFeaturing
      );
      this.m_fader.visible = true;
      ...
      Database.FeatureChallenge(
        ControllerGameGlobals.curChallengeID,
        !ControllerGameGlobals.curChallengeFeatured,
        this.finishFeaturing
      );
      this.m_fader.visible = true;
      this.ShowDialog("Featuring...");
      ...
      Database.FeatureRobot(
        ControllerGameGlobals.curRobotID,
        !ControllerGameGlobals.curRobotFeatured,
        this.finishFeaturing
      );
      this.m_fader.visible = true;
      this.ShowDialog("Featuring...");
  ...
  private finishFeaturing(): void {
    ...
    if (retVal) {
      ...
      if (ControllerGameGlobals.playingReplay)
        ...
        ControllerGameGlobals.curChallengeFeatured = !ControllerGameGlobals.curChallengeFeatured;
  ...
  public restrictionsButton(): void {
    if (this.selectingCondition) return;
    this.m_sidePanel.visible = false;
  ...
  public finishCommenting(): void {
    if (
      !Database.waitingForResponse ||
      (Database.curTransactionType != Database.ACTION_COMMENT_ROBOT &&
    ...
    if (threadID != -1) {
      ...
      Main.BrowserRedirect(
        "http://www.incredibots.com/forums/posting.php?mode=reply&t=" +
          threadID +
  ...
  public finishRatingRobot(): void {
    ...
    if (this.redirectAfterRating == 1) {
      ...
      this.loadReplayButton();
    } else if (this.redirectAfterRating == 4) {
      this.loadChallengeButton();
  ...
  public linkButton(robotID: string = "", robotPublic: boolean = false): void {
    ...
    if (robotID != "" && robotPublic) {
      ...
      this.ShowLinkDialog(
        ...
        "http://incredibots2.com/?robotID=" + robotID
      );
    } else {
      this.m_fader.visible = true;
      this.ShowDialog3("You need to save your robot publicly first!");
  ...
  public linkReplayButton(replayID: string = "", replayPublic: boolean = false): void {
    ...
    if (replayID != "" && replayPublic) {
      ...
      this.ShowLinkDialog(
        "     Use the link below to\n       link to this replay.",
        "http://incredibots2.com/?replayID=" + replayID
      );
    } else {
      this.m_fader.visible = true;
      this.ShowDialog3("You need to save your replay publicly first!");
  ...
  public rateChallengeButton(): void {
    if (this.selectingCondition) return;
    if (
      ControllerGameGlobals.curChallengeID != "" &&
      ControllerGameGlobals.curChallengePublic &&
  ...
  public linkChallengeButton(challengeID: string = "", challengePublic: boolean = false): void {
    ...
    if (challengeID != "" && challengePublic) {
      ...
      this.ShowLinkDialog(
        ...
        "http://incredibots2.com/?challengeID=" + challengeID
      );
    } else {
      this.m_fader.visible = true;
      this.ShowDialog3("You need to save your challenge publicly first!");
  ...
  public rotateButton(): void {
    ...
    for (i = 0; i < this.selectedParts.length; i++) {
      if (
        (this.selectedParts[i] instanceof ShapePart &&
          (!this.rotatingPart ||
            this.rotatingPart instanceof Thrusters ||
            this.selectedParts[i].GetMass() > this.rotatingPart.GetMass())) ||
        (this.selectedParts[i] instanceof Thrusters && !this.rotatingPart)
      ) {
    ...
    if (this.rotatingPart != null) {
      ...
      for (i = 0; i < this.rotatingParts.length; i++) {
        if (this.rotatingParts[i] instanceof ShapePart || this.rotatingParts[i] instanceof Thrusters) {
          this.rotatingParts[i].rotateAngle = Math.atan2(
            this.rotatingParts[i].centerY - this.rotatingPart.centerY,
            this.rotatingParts[i].centerX - this.rotatingPart.centerX
          );
          this.rotatingParts[i].rotateOrientation = this.rotatingParts[i].angle;
        } else {
          this.rotatingParts[i].rotateAngle = Math.atan2(
            this.rotatingParts[i].anchorY - this.rotatingPart.centerY,
            this.rotatingParts[i].anchorX - this.rotatingPart.centerX
          );
      ...
      this.initRotatingAngle = Math.atan2(
        ControllerGameGlobals.mouseYWorldPhys - this.rotatingPart.centerY,
        ControllerGameGlobals.mouseXWorldPhys - this.rotatingPart.centerX
      );
    }
  ...
  public mirrorHorizontal(): void {
    if (/*Main.premiumMode*/ true) {
      ...
      var centerX: number =
        this.selectedParts[0] instanceof JointPart
          ? this.selectedParts[0].anchorX
          : this.selectedParts[0] instanceof TextPart
          ? this.selectedParts[0].x + this.selectedParts[0].w / 2
          : this.selectedParts[0].centerX;
      var centerY: number =
        this.selectedParts[0] instanceof JointPart
          ? this.selectedParts[0].anchorY
          : this.selectedParts[0] instanceof TextPart
          ? this.selectedParts[0].y + this.selectedParts[0].h / 2
          : this.selectedParts[0].centerY;
      var partMapping: Array<any> = new Array();
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof Circle) {
          var c: Circle = new Circle(
            centerX - (this.selectedParts[i].centerX - centerX),
            this.selectedParts[i].centerY,
            this.selectedParts[i].radius
          );
          ...
          var r: Rectangle = new Rectangle(
            centerX - (this.selectedParts[i].x - centerX),
            this.selectedParts[i].y,
            -this.selectedParts[i].w,
            this.selectedParts[i].h
          );
          ...
          var ca: Cannon = new Cannon(
            centerX - (this.selectedParts[i].x - centerX) - this.selectedParts[i].w,
            this.selectedParts[i].y,
            this.selectedParts[i].w
          );
          ...
          var te: TextPart = new TextPart(
            this,
            centerX - (this.selectedParts[i].x + this.selectedParts[i].w / 2 - centerX),
            this.selectedParts[i].y,
            this.selectedParts[i].w,
            this.selectedParts[i].h,
            this.selectedParts[i].text,
            this.selectedParts[i].inFront
          );
      ...
      for (i = 0; i < this.selectedParts.length; i++) {
        ...
        if (this.selectedParts[i] instanceof FixedJoint) {
          var fj: FixedJoint = new FixedJoint(
            partMapping[index1],
            partMapping[index2],
            centerX - (this.selectedParts[i].anchorX - centerX),
            this.selectedParts[i].anchorY
          );
          newParts.push(fj);
        } else if (this.selectedParts[i] instanceof RevoluteJoint) {
          var rj: RevoluteJoint = new RevoluteJoint(
            partMapping[index1],
            partMapping[index2],
            centerX - (this.selectedParts[i].anchorX - centerX),
            this.selectedParts[i].anchorY
          ...
          var th: Thrusters = new Thrusters(
            partMapping[index1],
            centerX - (this.selectedParts[i].centerX - centerX),
            this.selectedParts[i].centerY
          );
      ...
      if (newParts.length > 0) {
        ...
        for (i = 0; i < newParts.length; i++) {
          newParts[i].dragXOff =
            centerX -
            (newParts[i] instanceof JointPart
              ? newParts[i].anchorX
              : newParts[i] instanceof TextPart
              ? newParts[i].x - newParts[i].w / 2
              : newParts[i].centerX);
          newParts[i].dragYOff =
            centerY -
            (newParts[i] instanceof JointPart
  ...
  public mirrorVertical(): void {
    if (/*Main.premiumMode*/ true) {
      ...
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof Circle) {
          var c: Circle = new Circle(
            this.selectedParts[i].centerX,
            centerY - (this.selectedParts[i].centerY - centerY),
            this.selectedParts[i].radius
          );
          ...
          var r: Rectangle = new Rectangle(
            this.selectedParts[i].x,
            centerY - (this.selectedParts[i].y - centerY),
            this.selectedParts[i].w,
            -this.selectedParts[i].h
          );
          ...
          var ca: Cannon = new Cannon(
            this.selectedParts[i].x,
            centerY - (this.selectedParts[i].y - centerY) - this.selectedParts[i].w / 2,
            this.selectedParts[i].w
          );
          ...
          var te: TextPart = new TextPart(
            this,
            this.selectedParts[i].x,
            centerY - (this.selectedParts[i].y + this.selectedParts[i].h / 2 - centerY),
            this.selectedParts[i].w,
            this.selectedParts[i].h,
            this.selectedParts[i].text,
            this.selectedParts[i].inFront
      ...
      for (i = 0; i < this.selectedParts.length; i++) {
        ...
        if (this.selectedParts[i] instanceof FixedJoint) {
          var fj: FixedJoint = new FixedJoint(
            partMapping[index1],
            partMapping[index2],
            this.selectedParts[i].anchorX,
            centerY - (this.selectedParts[i].anchorY - centerY)
          );
          newParts.push(fj);
        } else if (this.selectedParts[i] instanceof RevoluteJoint) {
          var rj: RevoluteJoint = new RevoluteJoint(
  ...
  public scaleButton(): void {
    if (/*Main.premiumMode*/ true) {
      ...
      for (i = 0; i < this.draggingParts.length; i++) {
        this.draggingParts[i].dragXOff =
          (this.draggingParts[i] instanceof JointPart
            ? this.draggingParts[i].anchorX
            : this.draggingParts[i] instanceof TextPart
            ? this.draggingParts[i].x + this.draggingParts[i].w / 2
            : this.draggingParts[i].centerX) - this.initDragX;
        this.draggingParts[i].dragYOff =
          (this.draggingParts[i] instanceof JointPart
            ? this.draggingParts[i].anchorY
            : this.draggingParts[i] instanceof TextPart
            ? this.draggingParts[i].y + this.draggingParts[i].h / 2
            : this.draggingParts[i].centerY) - this.initDragY;
  ...
  public deleteBuildBoxButton(): void {
    if (!ControllerGameGlobals.curRobotEditable) return;
    if (this.simStarted) return;
  ...
  public densitySlider(value: number): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof ShapePart) {
      ...
      (this.selectedParts[0] as ShapePart).density = value;
      ControllerGameGlobals.curRobotID = "";
      this.m_sidePanel.SetDensity(value);
      if (oldDensity != value)
        this.AddAction(
          new ChangeSliderAction(this.selectedParts[0], ChangeSliderAction.DENSITY_TYPE, value - oldDensity)
        );
    }
  }

  public densityText(value: string): void {
    if (this.lastSelectedShape instanceof ShapePart) {
      var oldDensity: number = (this.lastSelectedShape as ShapePart).density;
      var density: number = parseInt(value);
      ...
      if (oldDensity != density)
        this.AddAction(
          new ChangeSliderAction(this.selectedParts[0], ChangeSliderAction.DENSITY_TYPE, density - oldDensity)
        );
    }
  ...
  public strengthText(value: string): void {
    if (this.lastSelectedJoint instanceof JointPart) {
      ...
      if (this.lastSelectedJoint instanceof PrismaticJoint)
        oldStrength = (this.lastSelectedJoint as PrismaticJoint).pistonStrength;
      var strength: number = parseInt(value);
      if (this.lastSelectedJoint instanceof RevoluteJoint) {
      ...
      if (this.lastSelectedJoint instanceof RevoluteJoint)
        (this.lastSelectedJoint as RevoluteJoint).motorStrength = strength;
      if (this.lastSelectedJoint instanceof PrismaticJoint)
        (this.lastSelectedJoint as PrismaticJoint).pistonStrength = strength;
      ControllerGameGlobals.curRobotID = "";
      this.m_sidePanel.SetStrength(strength);
      if (oldStrength != strength)
        this.AddAction(
          new ChangeSliderAction(this.lastSelectedJoint, ChangeSliderAction.STRENGTH_TYPE, strength - oldStrength)
        );
    }
    this.m_sidePanel.TextAreaLostFocus();
  }
  ...
  public speedText(value: string): void {
    if (this.lastSelectedJoint instanceof JointPart) {
      ...
      if (this.lastSelectedJoint instanceof PrismaticJoint)
        oldSpeed = (this.lastSelectedJoint as PrismaticJoint).pistonSpeed;
      var speed: number = parseInt(value);
      if (this.lastSelectedJoint instanceof RevoluteJoint) {
      ...
      if (this.lastSelectedJoint instanceof PrismaticJoint)
        (this.lastSelectedJoint as PrismaticJoint).pistonSpeed = speed;
      ControllerGameGlobals.curRobotID = "";
      this.m_sidePanel.SetSpeed(speed);
      if (oldSpeed != speed)
        this.AddAction(new ChangeSliderAction(this.lastSelectedJoint, ChangeSliderAction.SPEED_TYPE, speed - oldSpeed));
    }
    this.m_sidePanel.TextAreaLostFocus();
  }

  public thrustText(value: string): void {
    if (this.lastSelectedThrusters instanceof Thrusters) {
      ...
      if (oldStrength != strength)
        this.AddAction(
          new ChangeSliderAction(this.lastSelectedThrusters, ChangeSliderAction.SPEED_TYPE, strength - oldStrength)
        );
    }
    this.m_sidePanel.TextAreaLostFocus();
  }

  public cannonText(value: string): void {
    if (this.lastSelectedShape instanceof Cannon) {
      ...
      if (oldStrength != strength)
        this.AddAction(
          new ChangeSliderAction(this.lastSelectedShape, ChangeSliderAction.STRENGTH_TYPE, strength - oldStrength)
        );
    }
    this.m_sidePanel.TextAreaLostFocus();
  }

  public cameraBox(value: boolean): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof ShapePart) {
      ...
      this.AddAction(new CameraAction(this.selectedParts[0], value, oldCameraPart));
    }
  }

  public collisionBox(value: boolean): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof ShapePart) {
      (this.selectedParts[0] as ShapePart).collide = value;
      ControllerGameGlobals.curRobotID = "";
      this.AddAction(new ShapeCheckboxAction(this.selectedParts[0], ShapeCheckboxAction.COLLIDE_TYPE, value));
    } else if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof PrismaticJoint) {
      (this.selectedParts[0] as PrismaticJoint).collide = value;
      ControllerGameGlobals.curRobotID = "";
      this.AddAction(new ShapeCheckboxAction(this.selectedParts[0], ShapeCheckboxAction.COLLIDE_TYPE, value));
      ...
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (
          (this.selectedParts[i] instanceof ShapePart || this.selectedParts[i] instanceof PrismaticJoint) &&
          this.selectedParts[i].collide != value
        ) {
          this.selectedParts[i].collide = value;
      ...
      if (affectedParts.length > 0) {
        ControllerGameGlobals.curRobotID = "";
        this.redrawRobot = true;
        this.AddAction(new MultiCollideAction(affectedParts, value));
      }
    }
  }
  ...
  public fixateBox(value: boolean): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof ShapePart) {
      ...
      var affectedParts: Array<any> = new Array();
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof ShapePart && this.selectedParts[i].isStatic != value) {
          this.selectedParts[i].isStatic = value;
          affectedParts.push(this.selectedParts[i]);
        }
      }
      if (affectedParts.length > 0) {
        ControllerGameGlobals.curRobotID = "";
        this.redrawRobot = true;
        this.AddAction(new MultiFixateAction(affectedParts, value));
      }
    }
  }

  public outlineBox(value: boolean): void {
    if (
      this.selectedParts.length == 1 &&
      (this.selectedParts[0] instanceof ShapePart || this.selectedParts[0] instanceof PrismaticJoint)
    ) {
      this.selectedParts[0].outline = value;
      ControllerGameGlobals.curRobotID = "";
      this.redrawRobot = true;
      this.AddAction(new ShapeCheckboxAction(this.selectedParts[0], ShapeCheckboxAction.OUTLINE_TYPE, value));
    } else if (this.selectedParts.length > 1) {
      var affectedParts: Array<any> = new Array();
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (
          (this.selectedParts[i] instanceof ShapePart || this.selectedParts[i] instanceof PrismaticJoint) &&
          this.selectedParts[i].outline != value
        ) {
          this.selectedParts[i].outline = value;
          affectedParts.push(this.selectedParts[i]);
        }
      ...
      if (affectedParts.length > 0) {
        ControllerGameGlobals.curRobotID = "";
        this.redrawRobot = true;
        this.AddAction(new MultiOutlineAction(affectedParts, value));
      }
    }
  }

  public terrainBox(value: boolean): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof ShapePart) {
      this.selectedParts[0].terrain = value;
      ControllerGameGlobals.curRobotID = "";
      this.redrawRobot = true;
      this.AddAction(new ShapeCheckboxAction(this.selectedParts[0], ShapeCheckboxAction.TERRAIN_TYPE, value));
    } else if (this.selectedParts.length > 1) {
      var affectedParts: Array<any> = new Array();
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof ShapePart && this.selectedParts[i].terrain != value) {
          this.selectedParts[i].terrain = value;
          affectedParts.push(this.selectedParts[i]);
        }
      }
      if (affectedParts.length > 0) {
        ControllerGameGlobals.curRobotID = "";
        this.redrawRobot = true;
        this.AddAction(new MultiTerrainAction(affectedParts, value));
      }
    }
  }

  public undragableBox(value: boolean): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof ShapePart) {
      this.selectedParts[0].undragable = value;
      ControllerGameGlobals.curRobotID = "";
      this.AddAction(new ShapeCheckboxAction(this.selectedParts[0], ShapeCheckboxAction.UNDRAGABLE_TYPE, value));
    } else if (this.selectedParts.length > 1) {
      var affectedParts: Array<any> = new Array();
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof ShapePart && this.selectedParts[i].undragable != value) {
          this.selectedParts[i].undragable = value;
          affectedParts.push(this.selectedParts[i]);
        }
      }
      if (affectedParts.length > 0) {
        ControllerGameGlobals.curRobotID = "";
        this.redrawRobot = true;
        this.AddAction(new MultiUndragableAction(affectedParts, value));
      }
    }
  }

  public frontButton(): void {
    if (this.simStarted) return;
    if (
      this.selectedParts.length == 1 &&
      (this.selectedParts[0] instanceof ShapePart || this.selectedParts[0] instanceof PrismaticJoint)
  ...
  public scaleWithZoomBox(value: boolean): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof TextPart) {
      ...
      ControllerGameGlobals.curRobotID = "";
      this.AddAction(new TextCheckboxAction(this.selectedParts[0], TextCheckboxAction.SCALE_TYPE, value));
    }
  }
  ...
  public alwaysVisibleBox(value: boolean): void {
    if (this.selectedParts.length == 1 && this.selectedParts[0] instanceof TextPart) {
      ...
      ControllerGameGlobals.curRobotID = "";
      this.m_sidePanel.EnableTextStuff(!value);
      this.AddAction(new TextCheckboxAction(this.selectedParts[0], TextCheckboxAction.DISPLAY_TYPE, value));
    }
  }
  ...
  public textKeyBox(input: GuiTextInput, keyNumber: number): void {
    if (input.enabled) {
      ...
      if (this.selectedParts[0] instanceof TextPart) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(new ControlKeyAction(this.selectedParts[0], ControlKeyAction.TEXT_TYPE, oldKey, keyNumber));
      }
  ...
  public colourButton(red: number, green: number, blue: number, opacity: number, defaultColour: boolean): void {
    ...
    if (this.selectedParts.length == 1) {
      if (
        this.selectedParts[0] instanceof ShapePart ||
        this.selectedParts[0] instanceof PrismaticJoint ||
        this.selectedParts[0] instanceof TextPart
      ) {
        ...
        if (
          ...
          oldGreen != this.selectedParts[0].green ||
          oldBlue != this.selectedParts[0].blue ||
          (!(this.selectedParts[0] instanceof TextPart) && oldOpacity != this.selectedParts[0].opacity)
        ) {
          this.AddAction(
            new ColourChangeAction(
              this.selectedParts[0],
              this.selectedParts[0].red - oldRed,
              this.selectedParts[0].green - oldGreen,
              this.selectedParts[0].blue - oldBlue,
              this.selectedParts[0] instanceof TextPart ? 0 : this.selectedParts[0].opacity - oldOpacity
            )
          );
          ControllerGameGlobals.curRobotID = "";
          this.redrawRobot = true;
      ...
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (
          this.selectedParts[i] instanceof ShapePart ||
          this.selectedParts[i] instanceof PrismaticJoint ||
          this.selectedParts[i] instanceof TextPart
        ) {
          if (
            this.selectedParts[i].red != red ||
            this.selectedParts[i].green != green ||
            this.selectedParts[i].blue != blue ||
            (!(this.selectedParts[i] instanceof TextPart) && this.selectedParts[i].opacity != opacity)
          ) {
  ...
  public sizeText(target: GuiTextInput, value: string): void {
    if (this.lastSelectedText instanceof TextPart) {
      ...
      ControllerGameGlobals.curRobotID = "";
      this.redrawRobot = true;
      if (oldSize != this.lastSelectedText.size)
        this.AddAction(new TextSizeChangeAction(this.lastSelectedText, this.lastSelectedText.size - oldSize));
    }
    this.m_sidePanel.TextAreaLostFocus();
  }
  ...
  public minLimitText(text: string): void {
    ...
    if (this.lastSelectedJoint instanceof RevoluteJoint) {
      ...
      if (limit != oldLimit)
        this.AddAction(new LimitChangeAction(this.lastSelectedJoint, LimitChangeAction.MIN_TYPE, oldLimit, limit));
    }
    this.m_sidePanel.TextAreaLostFocus();
    ControllerGameGlobals.curRobotID = "";
  }
  ...
  public maxLimitText(text: string): void {
    ...
    if (this.lastSelectedJoint instanceof RevoluteJoint) {
      ...
      if (limit != oldLimit)
        this.AddAction(new LimitChangeAction(this.lastSelectedJoint, LimitChangeAction.MAX_TYPE, oldLimit, limit));
    }
    this.m_sidePanel.TextAreaLostFocus();
    ControllerGameGlobals.curRobotID = "";
  }
  ...
  public controlKeyText1(input: GuiTextInput, keyNumber: number): void {
    if (input.enabled) {
      ...
      if (this.selectedParts[0] instanceof RevoluteJoint && keyNumber != null) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(new ControlKeyAction(this.selectedParts[0], ControlKeyAction.CW_TYPE, oldKey, keyNumber));
        ...
        if (oldKey != keyNumber)
          this.AddAction(new ControlKeyAction(this.selectedParts[0], ControlKeyAction.EXPAND_TYPE, oldKey, keyNumber));
      }
  ...
  public controlKeyText2(input: GuiTextInput, keyNumber: number): void {
    if (input.enabled) {
      ...
      if (this.selectedParts[0] instanceof RevoluteJoint && keyNumber != null) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(new ControlKeyAction(this.selectedParts[0], ControlKeyAction.CCW_TYPE, oldKey, keyNumber));
      } else if (keyNumber != null) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(
            new ControlKeyAction(this.selectedParts[0], ControlKeyAction.CONTRACT_TYPE, oldKey, keyNumber)
          );
      }
      if (keyNumber == null) str = "Unk: " + keyNumber;
      else str = Input.getKeyString(keyNumber);
  ...
  public thrustKeyText(input: GuiTextInput, keyNumber: number): void {
    if (input.enabled) {
      ...
      if (this.selectedParts[0] instanceof Thrusters && keyNumber != null) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(
            new ControlKeyAction(this.selectedParts[0], ControlKeyAction.THRUSTERS_TYPE, oldKey, keyNumber)
          );
      }
      if (keyNumber == null) str = "Unk: " + keyNumber;
      else str = Input.getKeyString(keyNumber);
  ...
  public fireKeyText(input: GuiTextInput, keyNumber: number): void {
    if (input.enabled) {
      ...
      if (this.selectedParts[0] instanceof Cannon && keyNumber != null) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(new ControlKeyAction(this.selectedParts[0], ControlKeyAction.CANNON_TYPE, oldKey, keyNumber));
      }
      if (keyNumber == null) str = "Unk: " + keyNumber;
      else str = Input.getKeyString(keyNumber);
      input.text = str;
  ...
  public autoBox1(value: boolean): void {
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      var sideEffect: boolean = value && (this.selectedParts[0] as RevoluteJoint).autoCCW;
      (this.selectedParts[0] as RevoluteJoint).autoCW = value;
      if (value) {
        (this.selectedParts[0] as RevoluteJoint).autoCCW = false;
        this.m_sidePanel.deselectBox2();
      }
      this.AddAction(
        new JointCheckboxAction(this.selectedParts[0], JointCheckboxAction.AUTO_CW_TYPE, value, sideEffect)
      );
    } else if (this.selectedParts[0] instanceof PrismaticJoint) {
      (this.selectedParts[0] as PrismaticJoint).autoOscillate = value;
      this.AddAction(new JointCheckboxAction(this.selectedParts[0], JointCheckboxAction.AUTO_OSCILLATE_TYPE, value));
    } else if (this.selectedParts[0] instanceof Thrusters) {
      (this.selectedParts[0] as Thrusters).autoOn = value;
      this.AddAction(new JointCheckboxAction(this.selectedParts[0], JointCheckboxAction.AUTO_ON_TYPE, value));
    }
    ControllerGameGlobals.curRobotID = "";
  }

  public autoBox2(value: boolean): void {
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      var sideEffect: boolean = value && (this.selectedParts[0] as RevoluteJoint).autoCW;
      (this.selectedParts[0] as RevoluteJoint).autoCCW = value;
      if (value) {
        (this.selectedParts[0] as RevoluteJoint).autoCW = false;
        this.m_sidePanel.deselectBox1();
      }
      this.AddAction(
        new JointCheckboxAction(this.selectedParts[0], JointCheckboxAction.AUTO_CCW_TYPE, value, sideEffect)
      );
    }
    ControllerGameGlobals.curRobotID = "";
  }

  public motorBox(value: boolean): void {
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      (this.selectedParts[0] as RevoluteJoint).enableMotor = value;
    } else if (this.selectedParts[0] instanceof PrismaticJoint) {
      (this.selectedParts[0] as PrismaticJoint).enablePiston = value;
    }
    this.m_sidePanel.EnableMotorStuff(value);
    ControllerGameGlobals.curRobotID = "";
    this.AddAction(new JointCheckboxAction(this.selectedParts[0], JointCheckboxAction.ENABLE_TYPE, value));
  }

  public rigidBox(value: boolean): void {
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      (this.selectedParts[0] as RevoluteJoint).isStiff = !value;
    } else if (this.selectedParts[0] instanceof PrismaticJoint) {
      (this.selectedParts[0] as PrismaticJoint).isStiff = !value;
    }
    ControllerGameGlobals.curRobotID = "";
    this.AddAction(new JointCheckboxAction(this.selectedParts[0], JointCheckboxAction.RIGID_TYPE, value));
  }

  public strengthSlider(value: number): void {
    var oldStrength: number;
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      oldStrength = (this.selectedParts[0] as RevoluteJoint).motorStrength;
      (this.selectedParts[0] as RevoluteJoint).motorStrength = value;
      if (oldStrength != value)
        this.AddAction(
          new ChangeSliderAction(this.selectedParts[0], ChangeSliderAction.STRENGTH_TYPE, value - oldStrength)
        );
    } else if (this.selectedParts[0] instanceof PrismaticJoint) {
      oldStrength = (this.selectedParts[0] as PrismaticJoint).pistonStrength;
      (this.selectedParts[0] as PrismaticJoint).pistonStrength = value;
      if (oldStrength != value)
        this.AddAction(
          new ChangeSliderAction(this.selectedParts[0], ChangeSliderAction.STRENGTH_TYPE, value - oldStrength)
        );
    ...
    ControllerGameGlobals.curRobotID = "";
    this.m_sidePanel.SetStrength(value);
  }

  public speedSlider(value: number): void {
    var oldSpeed: number;
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      oldSpeed = (this.selectedParts[0] as RevoluteJoint).motorSpeed;
      (this.selectedParts[0] as RevoluteJoint).motorSpeed = value;
      if (oldSpeed != value)
        this.AddAction(new ChangeSliderAction(this.selectedParts[0], ChangeSliderAction.SPEED_TYPE, value - oldSpeed));
    } else if (this.selectedParts[0] instanceof PrismaticJoint) {
    ...
    ControllerGameGlobals.curRobotID = "";
    this.m_sidePanel.SetSpeed(value);
  }

  public thrustSlider(value: number): void {
    var oldStrength: number;
    if (this.selectedParts[0] instanceof Thrusters) {
      oldStrength = (this.selectedParts[0] as Thrusters).strength;
      (this.selectedParts[0] as Thrusters).strength = value;
      if (oldStrength != value)
        this.AddAction(
          new ChangeSliderAction(this.selectedParts[0], ChangeSliderAction.STRENGTH_TYPE, value - oldStrength)
        );
    ...
    ControllerGameGlobals.curRobotID = "";
    this.m_sidePanel.SetThrust(value);
  }

  public cannonSlider(value: number): void {
    var oldStrength: number;
    if (this.selectedParts[0] instanceof Cannon) {
      oldStrength = (this.selectedParts[0] as Cannon).strength;
      (this.selectedParts[0] as Cannon).strength = value;
      if (oldStrength != value)
        this.AddAction(
          new ChangeSliderAction(this.selectedParts[0], ChangeSliderAction.STRENGTH_TYPE, value - oldStrength)
        );
      ...
      for (i = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof JointPart) {
          ...
          if (index1 != -1 && index2 != -1)
            ControllerGameGlobals.clipboardParts.push(
              (this.selectedParts[i] as JointPart).MakeCopy(partMapping[index1], partMapping[index2])
            );
          this.DeletePart(this.selectedParts[i], false, false);
        } else if (this.selectedParts[i] instanceof Thrusters) {
          index1 = -1;
          ...
          if (index1 != -1)
            ControllerGameGlobals.clipboardParts.push(
              (this.selectedParts[i] as Thrusters).MakeCopy(partMapping[index1])
            );
          this.DeletePart(this.selectedParts[i], false, false);
        }
      }
  ...
  public pasteButton(): void {
    ...
    if (
      ControllerGameGlobals.clipboardParts.length == 1 &&
      ControllerGameGlobals.clipboardParts[0] instanceof ShapePart
    ) {
      ControllerGameGlobals.clipboardParts[0].Move(
        ControllerGameGlobals.mouseXWorldPhys,
        ControllerGameGlobals.mouseYWorldPhys
      );
      ...
      if (ControllerGameGlobals.clipboardParts[0] instanceof Cannon)
        this.m_sidePanel.ShowCannonPanel(ControllerGameGlobals.clipboardParts[0]);
      ...
      for (i = 0; i < ControllerGameGlobals.clipboardParts.length; i++) {
        if (ControllerGameGlobals.clipboardParts[i] instanceof ShapePart) {
          if (
            !this.draggingPart ||
            this.draggingPart instanceof TextPart ||
            ControllerGameGlobals.clipboardParts[i].GetMass() > (this.draggingPart as ShapePart).GetMass()
          )
      ...
      for (i = 0; i < ControllerGameGlobals.clipboardParts.length; i++) {
        if (ControllerGameGlobals.clipboardParts[i] instanceof ShapePart) {
          ControllerGameGlobals.clipboardParts[i].dragXOff =
            (this.draggingPart as ShapePart).centerX - ControllerGameGlobals.clipboardParts[i].centerX;
          ControllerGameGlobals.clipboardParts[i].dragYOff =
            (this.draggingPart as ShapePart).centerY - ControllerGameGlobals.clipboardParts[i].centerY;
          this.allParts.push(ControllerGameGlobals.clipboardParts[i]);
          this.selectedParts.push(ControllerGameGlobals.clipboardParts[i]);
          this.draggingParts = Util.RemoveDuplicates(
            this.draggingParts.concat(ControllerGameGlobals.clipboardParts[i].GetAttachedParts())
          );
          ...
          if (oldClipboardParts[i] != this.draggingPart)
            oldClipboardParts[i].Move(
              oldClipboardParts[i].centerX -
                (this.draggingPart as ShapePart).centerX +
                ControllerGameGlobals.mouseXWorldPhys,
              oldClipboardParts[i].centerY -
                (this.draggingPart as ShapePart).centerY +
                ControllerGameGlobals.mouseYWorldPhys
            );
        } else if (ControllerGameGlobals.clipboardParts[i] instanceof TextPart) {
          if (this.draggingPart instanceof ShapePart) {
            ControllerGameGlobals.clipboardParts[i].dragXOff =
              (this.draggingPart as ShapePart).centerX - ControllerGameGlobals.clipboardParts[i].x;
            ControllerGameGlobals.clipboardParts[i].dragYOff =
              (this.draggingPart as ShapePart).centerY - ControllerGameGlobals.clipboardParts[i].y;
          } else {
            ControllerGameGlobals.clipboardParts[i].dragXOff =
              (this.draggingPart as TextPart).x - ControllerGameGlobals.clipboardParts[i].x;
            ControllerGameGlobals.clipboardParts[i].dragYOff =
              (this.draggingPart as TextPart).y - ControllerGameGlobals.clipboardParts[i].y;
          }
          this.allParts.push(ControllerGameGlobals.clipboardParts[i]);
          this.selectedParts.push(ControllerGameGlobals.clipboardParts[i]);
          ...
          if (oldClipboardParts[i] != this.draggingPart) {
            if (this.draggingPart instanceof ShapePart) {
              oldClipboardParts[i].Move(
                oldClipboardParts[i].x -
                  (this.draggingPart as ShapePart).centerX +
                  ControllerGameGlobals.mouseXWorldPhys,
                oldClipboardParts[i].y -
                  (this.draggingPart as ShapePart).centerY +
                  ControllerGameGlobals.mouseYWorldPhys
              );
            } else {
              oldClipboardParts[i].Move(
                oldClipboardParts[i].x - (this.draggingPart as TextPart).x + ControllerGameGlobals.mouseXWorldPhys,
                oldClipboardParts[i].y - (this.draggingPart as TextPart).y + ControllerGameGlobals.mouseYWorldPhys
              );
            }
          }
        }
      ...
      for (i = 0; i < ControllerGameGlobals.clipboardParts.length; i++) {
        if (ControllerGameGlobals.clipboardParts[i] instanceof JointPart) {
          ControllerGameGlobals.clipboardParts[i].dragXOff =
            (this.draggingPart as ShapePart).centerX - ControllerGameGlobals.clipboardParts[i].anchorX;
          ControllerGameGlobals.clipboardParts[i].dragYOff =
            (this.draggingPart as ShapePart).centerY - ControllerGameGlobals.clipboardParts[i].anchorY;
          this.allParts.push(ControllerGameGlobals.clipboardParts[i]);
          this.selectedParts.push(ControllerGameGlobals.clipboardParts[i]);
          var index1: number = -1,
            index2: number = -1;
          ...
          ControllerGameGlobals.clipboardParts[i] = ControllerGameGlobals.clipboardParts[i].MakeCopy(
            ControllerGameGlobals.clipboardParts[index1],
            ControllerGameGlobals.clipboardParts[index2]
          );
          oldClipboardParts[i].Move(
            oldClipboardParts[i].anchorX -
              (this.draggingPart as ShapePart).centerX +
              ControllerGameGlobals.mouseXWorldPhys,
            oldClipboardParts[i].anchorY -
              (this.draggingPart as ShapePart).centerY +
              ControllerGameGlobals.mouseYWorldPhys
          );
      ...
      if (this.m_chooserWindow.visible) {
        ...
        if (this.m_chooserWindow.dataType == SaveLoadWindow.HIGH_SCORE_TYPE) {
          Database.GetScoreData(
            ControllerGameGlobals.userName,
            ControllerGameGlobals.password,
            Database.highScoresChallenge,
            this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 1,
            this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 2,
            success && this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 2
              ? Database.SORT_BY_SCORE
              : Database.highScoresSortType,
            success && this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 2 ? 1 : Database.curScorePage,
            success && this.m_chooserWindow.m_scoreTypeBox && this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 2
              ? ""
              : Database.curSearch,
            this.finishGettingScoreData
          );
          this.ShowDialog("Getting scores...");
        } else if (this.m_chooserWindow.dataType == SaveLoadWindow.LOAD_ROBOT_TYPE) {
          Database.GetRobotData(
            ControllerGameGlobals.userName,
            ControllerGameGlobals.password,
            !this.m_chooserWindow.yourRobotsBox.selected,
            success && this.m_chooserWindow.yourRobotsBox.selected ? Database.SORT_BY_EDIT_TIME : Database.curSortType,
            success && this.m_chooserWindow.yourRobotsBox.selected
              ? Database.SORT_PERIOD_ALLTIME
              : Database.curSortPeriod,
            success && this.m_chooserWindow.yourRobotsBox.selected ? 1 : Database.curRobotPage,
            success && this.m_chooserWindow.m_scoreTypeBox && this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 2
              ? ""
              : Database.curSearch,
          ...
          if (Database.curSortPeriod == Database.SORT_PERIOD_PROP)
            Database.curSortPeriod = Database.SORT_PERIOD_ALLTIME;
          Database.GetChallengeData(
            ...
            success && this.m_chooserWindow.yourRobotsBox.selected
              ? Database.SORT_PERIOD_ALLTIME
              : Database.curSortPeriod,
            success && this.m_chooserWindow.yourRobotsBox.selected ? 1 : Database.curReplayPage,
            success && this.m_chooserWindow.m_scoreTypeBox && this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 2
              ? ""
              : Database.curSearch,
  ...
  public viewReplayButton(): void {
    if (ControllerGameGlobals.viewingUnsavedReplay) {
      ...
      this.resetButton();
      ControllerGameGlobals.replay = new Replay(
        this.cameraMovements,
        this.syncPoints,
        this.keyPresses,
        this.frameCounter,
  ...
  public loadRobotButton(makeThemRate: boolean = true): void {
    ...
    if (Main.inIFrame) {
      ...
      Database.GetRobotData(
        ...
        ControllerGameGlobals.password,
        Database.curShared,
        Database.curSortType,
        Database.curSortPeriod,
        Database.curShared ? Database.curRobotPage : 1,
        "",
        this.finishGettingLoadRobotData
      );
      this.m_fader.visible = true;
    ...
    if (robot) {
      ...
      if (ControllerGameGlobals.loadAndInsert) {
        ...
        if (
          ...
          this.m_fader.visible = false;
          ControllerGameGlobals.curRobotEditable =
            ControllerGameGlobals.potentialRobotEditable /* && ((!hasThrusters && !hasCannons) || Main.premiumMode)*/;
          ControllerGameGlobals.curRobotPublic = false;
  ...
  public saveReplayButton(): void {
    if (Main.inIFrame) {
      ...
      if (this.canSaveReplay) {
        ...
        this.m_fader.visible = true;
        if (this.frameCounter >= 9000)
          this.ShowDialog3("Sorry, you can only save replays that are under 5 minutes in length.");
  ...
  public loadReplayButton(makeThemRate: boolean = true): void {
    ...
    if (Main.inIFrame) {
      ...
      Database.GetReplayData(
        ...
        Database.curSortType,
        Database.curSortPeriod,
        Database.curShared ? Database.curReplayPage : 1,
        "",
        this.finishGettingLoadReplayData
      );
      this.ShowDialog("Getting replays...");
    ...
    if (Main.inIFrame) {
      ...
      if (ControllerGameGlobals.curChallengeID == "") {
        ...
        Database.GetScoreData(
          ...
          ControllerGameGlobals.password,
          ControllerGameGlobals.curChallengeID,
          Database.highScoresDaily,
          Database.highScoresPersonal,
          Database.highScoresSortType,
          Database.curScorePage,
          "",
          this.finishGettingScoreData
        );
    ...
    if (replayAndRobot) {
      ...
      if (ControllerGameGlobals.replay.version != Database.VERSION_STRING_FOR_REPLAYS) {
        if (ControllerGameGlobals.replayDirectlyLinked) {
          if (Main.inIFrame)
            Main.BrowserRedirect(
              "http://incredibots.com/old/" +
                ControllerGameGlobals.replay.version +
                "/incredibots.php?replayID=" +
                ControllerGameGlobals.potentialReplayID
            );
          else
            Main.BrowserRedirect(
              "http://incredibots.com/old/" +
  ...
  protected FindCenterOfRobot(): ShapePart {
    ...
    for (var i: number = 0; i < this.allParts.length; i++) {
      if (
        this.allParts[i].isEditable &&
        this.allParts[i] instanceof ShapePart &&
        this.allParts[i].isEnabled &&
        this.allParts[i].m_collisionGroup == -(i + 1)
      ) {
  ...
  private Zoom(zoomIn: boolean): void {
    ...
    if (this.simStarted)
      this.cameraMovements.push(
        new CameraMovement(
          this.frameCounter,
          this.autoPanning ? Number.POSITIVE_INFINITY : this.draw.m_drawXOff,
          this.autoPanning ? Number.POSITIVE_INFINITY : this.draw.m_drawYOff,
          this.m_physScale
        )
      );
  }
  ...
  private MouseOverSelectedPart(): boolean {
    for (var i: number = 0; i < this.selectedParts.length; i++) {
      if (
        this.selectedParts[i].InsideShape(
          ControllerGameGlobals.mouseXWorldPhys,
          ControllerGameGlobals.mouseYWorldPhys,
          this.m_physScale
        )
  ...
  private GetPartAtMouse(): Part {
    for (var j: number = 0; j < this.allParts.length; j++) {
      if (
        this.allParts[j].isEditable &&
        this.allParts[j] instanceof TextPart &&
        this.allParts[j].InsideMoveBox(
          ControllerGameGlobals.mouseXWorldPhys,
          ControllerGameGlobals.mouseYWorldPhys,
          this.m_physScale
        )
      )
        return this.allParts[j];
    }
