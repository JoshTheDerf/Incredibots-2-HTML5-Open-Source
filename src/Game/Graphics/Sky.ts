...
export class Sky {
  private static topColours = [
    ...
    "#292935",
    "#22243C",
    "#BE4A3C" /*#DD8163*/ /*#C2603C*/,
    /*#58ACB4*/ /*#5072BC*/ "#4790EC",
  ];
  private static bottomColours = [
    "#8ACEF7",
    "#353738",
    "#48475F",
    "#40457E",
    "#EB7A76" /*#EDAD8D*/ /*#D7A354*/,
    /*#E47B4C*/ /*#E1A64C*/ "#EC8D66",
  ];
  ...
  constructor(contr: Controller, type: number, r: number = 0, g: number = 0, b: number = 0) {
    ...
    const gradientSprite = new Sprite(
      Gradient.getLinearGradientTexture(
        type == SandboxSettings.BACKGROUND_SOLID_COLOUR
          ? [Util.HexColourString(r, g, b), Util.HexColourString(r, g, b)]
          : [Sky.topColours[type], Sky.bottomColours[type]]
      )
    );
    ...
    if (type == 0) {
      ...
      for (var i: number = 0; i < this.numClouds; i++) {
        var cloud: Sprite = new Sprite(Resource.cCloud0);
        ...
        this.cloudPositions.push(
          new b2Vec2(Util.RangedRandom(this.cont.GetMinX() - 5, this.cont.GetMaxX() + 5), Util.RangedRandom(-15, 6))
        );
  ...
  public Update(hasZoomed: boolean, hasPanned: boolean): void {
    if (hasZoomed) {
      if (this.skyType == 0) {
        for (var i: number = 0; i < this.numClouds; i++) {
          this.sClouds[i].width = (this.sClouds[i].texture.width * this.cont.GetPhysScale()) / 90;
          this.sClouds[i].height = (this.sClouds[i].texture.height * this.cont.GetPhysScale()) / 90;
        }
      } else if (this.skyType == 1) {
        this.sClouds[0].width =
          this.cont.World2ScreenX(this.cont.GetMaxX() + 5) - this.cont.World2ScreenX(this.cont.GetMinX() - 5);
