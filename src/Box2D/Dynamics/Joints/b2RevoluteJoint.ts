...
import { b2Joint, b2Vec2, b2RevoluteJointDef, b2Mat22, b2TimeStep, b2Body, b2Math, b2Settings } from "../..";
...
export class b2RevoluteJoint extends b2Joint
{
	...
	public InitVelocityConstraints(step:b2TimeStep) : void{
		...
		if (this.m_enableLimit)
		{
			...
			var jointAngle:number = b2.m_sweep.a - b1.m_sweep.a - this.m_referenceAngle;
	...
	public SolveVelocityConstraints(step:b2TimeStep) : void{
		var b1:b2Body = this.m_body1, b2:b2Body = this.m_body2, tMat:b2Mat22, tX:number;

		//b2Vec2 r1 = b2Mul(b1->m_xf.R, m_localAnchor1 - b1->GetLocalCenter());
		tMat = b1.m_xf.R;
		var r1X:number = this.m_localAnchor1.x - b1.m_sweep.localCenter.x, r1Y:number = this.m_localAnchor1.y - b1.m_sweep.localCenter.y;
		tX =  (tMat.col1.x * r1X + tMat.col2.x * r1Y);
		r1Y = (tMat.col1.y * r1X + tMat.col2.y * r1Y);
		r1X = tX;
		//b2Vec2 r2 = b2Mul(b2->m_xf.R, m_localAnchor2 - b2->GetLocalCenter());
		tMat = b2.m_xf.R;
		var r2X:number = this.m_localAnchor2.x - b2.m_sweep.localCenter.x, r2Y:number = this.m_localAnchor2.y - b2.m_sweep.localCenter.y;
		tX =  (tMat.col1.x * r2X + tMat.col2.x * r2Y);
		r2Y = (tMat.col1.y * r2X + tMat.col2.y * r2Y);
		r2X = tX;
		...
		pivotCdotX:number = b2.m_linearVelocity.x + (-b2.m_angularVelocity * r2Y) - b1.m_linearVelocity.x - (-b1.m_angularVelocity * r1Y), pivotCdotY:number = b2.m_linearVelocity.y + (b2.m_angularVelocity * r2X) - b1.m_linearVelocity.y - (b1.m_angularVelocity * r1X);

		//b2Vec2 pivotForce = -step.inv_dt * b2Mul(m_pivotMass, pivotCdot);
		var pivotForceX:number = -step.inv_dt * (this.m_pivotMass.col1.x * pivotCdotX + this.m_pivotMass.col2.x * pivotCdotY), pivotForceY:number = -step.inv_dt * (this.m_pivotMass.col1.y * pivotCdotX + this.m_pivotMass.col2.y * pivotCdotY);
		this.m_pivotForce.x += pivotForceX;
		this.m_pivotForce.y += pivotForceY;

		//b2Vec2 P = step.dt * pivotForce;
		var PX:number = step.dt * pivotForceX, PY:number = step.dt * pivotForceY;
		...
		{
			var motorCdot:number = b2.m_angularVelocity - b1.m_angularVelocity - this.m_motorSpeed;
			var motorForce:number = -step.inv_dt * this.m_motorMass * motorCdot, oldMotorForce:number = this.m_motorForce;
	...
	public SolvePositionConstraints():boolean{
		var oldLimitImpulse:number, limitC:number, b1:b2Body = this.m_body1, b2:b2Body = this.m_body2, positionError:number = 0.0, tMat:b2Mat22;
		...
		//b2Vec2 ptpC = p2 - p1;
		var ptpCX:number = p2X - p1X, ptpCY:number = p2Y - p1Y;
		...
		invI1:number = b1.m_invI, invI2:number = b2.m_invI;
		...
		var impulseX:number = b2RevoluteJoint.tImpulse.x, impulseY:number = b2RevoluteJoint.tImpulse.y;
		...
		if (this.m_enableLimit && this.m_limitState != b2RevoluteJoint.e_inactiveLimit)
		{
			var angle:number = b2.m_sweep.a - b1.m_sweep.a - this.m_referenceAngle, limitImpulse:number = 0.0;
