...
import { b2Joint, b2Vec2, b2Mat22, b2GearJointDef, b2RevoluteJoint, b2PrismaticJoint, b2TimeStep, b2Body, b2Settings, b2Jacobian } from "../..";
...
export class b2GearJoint extends b2Joint
{
	...
	public GetReactionForce():b2Vec2{
		...
		var F:b2Vec2 = new b2Vec2(this.m_force * this.m_J.linear2.x, this.m_force * this.m_J.linear2.y);
		return F;
	}
	public GetReactionTorque():number{
		// TODO_ERIN not tested
		//b2Vec2 r = b2Mul(m_body2->m_xf.R, m_localAnchor2 - m_body2->GetLocalCenter());
		var tMat:b2Mat22 = this.m_body2.m_xf.R;
		var rX:number = this.m_localAnchor1.x - this.m_body2.m_sweep.localCenter.x;
		var rY:number = this.m_localAnchor1.y - this.m_body2.m_sweep.localCenter.y;
	...
	public InitVelocityConstraints(step:b2TimeStep) : void{
		...
		{
			// Warm starting.
			var P:number = step.dt * this.m_force;
	...
	public SolveVelocityConstraints(step:b2TimeStep): void{
		...
		var Cdot:number = this.m_J.Compute(	b1.m_linearVelocity, b1.m_angularVelocity,
