...
import { b2BroadPhase, b2BufferedPair, b2Pair, b2PairCallback, b2Proxy, b2Settings } from "..";
...
//public:
	...
	*/
	public AddBufferedPair(proxyId1:number, proxyId2:number) : void{
	...
	public RemoveBufferedPair(proxyId1:number, proxyId2:number) : void{
		var bufferedPair:b2BufferedPair;
	...
	public Commit() : void{
		...
		var proxies:Array<any> = this.m_broadPhase.m_proxyPool;

		for (i = 0; i < this.m_pairBufferCount; ++i)
		{
			bufferedPair = this.m_pairBuffer[i];
			var pair:b2Pair = this.Find(bufferedPair.proxyId1, bufferedPair.proxyId2);
...
//private:
	...
	private AddPair(proxyId1:number, proxyId2:number):b2Pair{
		...
		//var pairIndex:number = FindHash(proxyId1, proxyId2, hash);
		var pair:b2Pair = this.FindHash(proxyId1, proxyId2, hash);
	...
	private RemovePair(proxyId1:number, proxyId2:number):any{
		var pair:b2Pair;
		...
		while (node != b2Pair.b2_nullPair)
		{
			if (b2PairManager.Equals(this.m_pairs[node], proxyId1, proxyId2))
			{
				...
				var userData:any = pair.userData;
	...
	private Find(proxyId1:number, proxyId2:number):b2Pair{
	...
	private FindHash(proxyId1:number, proxyId2:number, hash:number):b2Pair{
		var pair:b2Pair;
		var index:number = this.m_hashTable[hash];
...
//public:
	...
	public m_pairBuffer:Array<any>;
	public m_pairBufferCount:number;

	public m_hashTable:Array<any>;
...
// static
	...
	{
		var key:number = ((proxyId2 << 16) & 0xffff0000) | proxyId1;
