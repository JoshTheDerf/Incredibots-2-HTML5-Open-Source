package Gui
{
	...
	public class TutorialWindow extends GuiWindow
	{
		private var cont:ControllerGame;
		private var msgArea:TextField;
		private var msgArea2:TextField;
		private var msgArea3:TextField;
		private var num:int;
		private var initX:Number;
		private var initY:Number;

		public function TutorialWindow(contr:ControllerGame, x:int, y:int, id:int, moreButton:Boolean = false)
		{
			var big:Boolean = (id == 1 || id == 4 || id == 14 || id == 21 || id == 23 || id == 27 || id == 30 || id == 44 || id == 48 || id == 50 || id == 52 || id == 53 || id == 56 || id == 57 || id == 58 || id == 60 || (id > 70 && id < 82));
			if (id >= 70 && x < 100) {
				if (x < 50) x = 50;
			} else if (x < 100) {
				x = 100;
			...
			if (y < 120) y = 120;
			if (y > 380) y = 380;
			super(x, y, 248, (id == 25 ? 185 : (big ? 180 : 170)));
			initX = x;
			initY = y;
			cont = contr;
			num = id;
			
			var msg:String = TUTORIAL_MESSAGES[id];
			var numAreas:int = 1;
			while (msg.search("\n\n") != -1) {
				numAreas++;
			...
			msg = TUTORIAL_MESSAGES[id];
			
			msgArea = new TextField();
			msgArea.x = 10;
			msgArea.y = 16;
			msgArea.width = 228;
			msgArea.text = (numAreas == 1 ? msg : msg.substr(0, msg.search("\n\n")));
			...
			if (numAreas > 1) {
				msgArea2 = new TextField();
				msgArea2.x = 10;
				var numLines:int = 1;
				var tempMsg:String = msgArea.text;
				while (tempMsg.search("\r") != -1) {
					numLines++;
					tempMsg = tempMsg.substr(tempMsg.search("\r") + 2);
				}
				msgArea2.y = msgArea.y + 10 + (numLines * 16);
				msgArea2.width = 228;
				msgArea2.text = (numAreas == 2 ? msg : msg.substr(0, msg.search("\n\n")));
				msgArea2.textColor = 0x242930;
				msgArea2.selectable = false;
				format = new TextFormat();
				format.size = 12;
				format.align = TextFormatAlign.CENTER;
				format.font = Main.GLOBAL_FONT;
				format.leading = 2;
				msgArea2.setTextFormat(format);
				addChild(msgArea2);
			}
			...
			if (numAreas > 2) {
				msgArea3 = new TextField();
				msgArea3.x = 10;
				numLines = 1;
				tempMsg = msgArea2.text;
				while (tempMsg.search("\r") != -1) {
					numLines++;
					tempMsg = tempMsg.substr(tempMsg.search("\r") + 2);
				}
				msgArea3.y = msgArea2.y + 10 + (numLines * 16);
				msgArea3.width = 228;
				msgArea3.text = msg;
				msgArea3.textColor = 0x242930;
				msgArea3.selectable = false;
				format = new TextFormat();
				format.size = 12;
				format.align = TextFormatAlign.CENTER;
				format.font = Main.GLOBAL_FONT;
				format.leading = 2;
				msgArea3.setTextFormat(format);
				addChild(msgArea3);
			}

			var b:GuiButton = new GuiButton((moreButton ? "More..." : "OK"), 89, (id == 25 ? 140 : (big ? 135 : 125)), 70, 40, closeWindow, GuiButton.PURPLE);
			addChild(b);
		}
		...
		public function closeWindow(e:MouseEvent):void {
			cont.CloseTutorialDialog(num);
		}
		
		private var TUTORIAL_MESSAGES:Array =  ["Welcome to the world of IncrediBots 2!\n\nThis is the robot building screen,\nwhere you'll learn to construct\namazing robots like this tank... or\nanything else you can imagine!",
