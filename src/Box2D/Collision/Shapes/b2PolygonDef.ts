...
import { b2ShapeDef, b2Shape, b2Settings, b2Vec2, b2Mat22 } from "../..";
...
export class b2PolygonDef extends b2ShapeDef
{
	public b2PolygonDef()
	{
		...
		for (var i:number = 0; i < b2Settings.b2_maxPolygonVertices; i++){
	...
	public SetAsOrientedBox(hx:number, hy:number, center:b2Vec2=null, angle:number=0.0) : void{
		...
		if (center){
			...
			//xf.R.Set(angle);
			var xfR:b2Mat22 = b2PolygonDef.s_mat;
			xfR.Set(angle);
	...
	public vertices:Array<any>= new Array(b2Settings.b2_maxPolygonVertices);
