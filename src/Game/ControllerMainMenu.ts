...
export class ControllerMainMenu extends Controller {
  ...
  constructor(straightToLevelSelect: boolean = false) {
    ...
    const gradient1 = Gradient.getLinearGradientTexture([
      Util.HexColourString(177, 102, 46),
      Util.HexColourString(171, 59, 34),
    ]);
    const gradient2 = Gradient.getLinearGradientTexture([
    ...
    style = new TextStyle();
    style.fontSize = 17;
    var button: GuiButton = new GuiButton(
      "Tutorial Levels",
      210,
      109,
      200,
      65,
      this.tutorialButton.bind(this),
      GuiButton.PINK,
      style
    );
    ...
    this.enableSoundButton = new GuiButton(
      "Enable Sound",
      5,
      535,
      150,
      60,
      this.enableSoundButtonPressed.bind(this),
      GuiButton.BLUE,
      style
    );
    this.enableSoundButton.visible = !Main.enableSound;
    this.disableSoundButton = new GuiButton(
      "Disable Sound",
      5,
      535,
      150,
      60,
      this.disableSoundButtonPressed.bind(this),
      GuiButton.BLUE,
      style
    );
    ...
    button = new GuiButton(
      ...
      171,
      59,
      this.importChallengeButton.bind(this),
      GuiButton.ORANGE,
      style
    );
    this.levelSelectGui.addChild(button);
    button = new GuiButton(
      ...
      171,
      59,
      this.importReplayButton.bind(this),
      GuiButton.ORANGE,
      style
    );
    this.levelSelectGui.addChild(button);
    button = new GuiButton(
      ...
      171,
      59,
      (event: any) => this.importRobotButton(event),
      GuiButton.ORANGE,
      style
    );
    this.levelSelectGui.addChild(button);
    ...
    this.LoadReplay().then(() => {
      ...
      for (i = 0; i < this.allParts.length; i++) {
        if (this.allParts[i] instanceof JointPart || this.allParts[i] instanceof Thrusters)
          this.allParts[i].Init(this.world);
      }
  ...
  private loadChallengeButton(e: MouseEvent): void {
    ...
    Database.GetChallengeData(
      ...
      Database.curSortType,
      Database.curSortPeriod,
      Database.curShared ? Database.curChallengePage : 1,
      "",
  ...
  public processLoadedRobot(robot: Robot): void {
    ...
    ControllerGameGlobals.curRobotEditable =
      ControllerGameGlobals.potentialRobotEditable /* && (!hasThrusters || Main.premiumMode)*/;
  ...
  public ReloadLoadWindow(): void {
    ...
    if (this.m_loadWindow.dataType != SaveLoadWindow.HIGH_SCORE_TYPE && this.m_loadWindow.yourRobotsBox.selected) {
      ...
      Database.GetScoreData(
        ...
        this.m_loadWindow.m_scoreTypeBox.selectedIndex == 1,
        this.m_loadWindow.m_scoreTypeBox.selectedIndex == 2,
  ...
  public finishCommenting(e: Event): void {
    if (
      ...
      (Database.curTransactionType != Database.ACTION_COMMENT_ROBOT &&
        ...
        Database.curTransactionType != Database.ACTION_COMMENT_CHALLENGE)
    )
      return;
    ...
    if (threadID != -1) {
      ...
      Main.BrowserRedirect(
        "http://incredibots.com/forums/posting.php?mode=reply&t=" +
          threadID +
  ...
  public HideDialog(e: Event): void {
    ...
    this.m_progressDialog.visible = false;
    if (
      (!this.m_loadWindow || !this.m_loadWindow.visible) &&
      (!this.m_newUserWindow || !this.m_newUserWindow.visible) &&
      (!this.m_loginWindow || !this.m_loginWindow.visible) &&
      (!this.m_challengeWindow || !this.m_challengeWindow.visible)
    ) {
      this.fader2.visible = false;
    }
  }
  ...
  private ComputeReplaySplines(type: number): Array<any> {
    ...
    for (var i: number = 0; i < this.replay.syncPoints.length - 1; i++) {
      ...
      for (var j: number = 0; j < this.replay.syncPoints[0].positions.length; j++) {
        if (type == 0)
          inner.push((this.replay.syncPoints[i + 1].positions[j].x - this.replay.syncPoints[i].positions[j].x) / h[i]);
        else if (type == 1)
          inner.push((this.replay.syncPoints[i + 1].positions[j].y - this.replay.syncPoints[i].positions[j].y) / h[i]);
        else {
    ...
    for (i = 0; i < this.replay.syncPoints.length - 1; i++) {
      ...
      for (j = 0; j < this.replay.syncPoints[0].positions.length; j++) {
        innerA.push(
          type == 0
            ? this.replay.syncPoints[i].positions[j].x
            : type == 1
            ? this.replay.syncPoints[i].positions[j].y
            : this.replay.syncPoints[i].angles[j]
        );
        ...
        innerB.push(
          (-h[i] * z[i + 1][j]) / 6 -
            (h[i] * z[i][j]) / 3 +
            (type == 0
              ? this.replay.syncPoints[i + 1].positions[j].x - innerA[j]
              : type == 1
              ? this.replay.syncPoints[i + 1].positions[j].y - innerA[j]
              : deltaAngle) /
              h[i]
        );
        innerC.push(z[i][j] / 2);
