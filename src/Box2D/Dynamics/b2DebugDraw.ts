...
export class b2DebugDraw
{
	...
	public DrawPolygon(vertices:Array<any>, vertexCount:number, color:b2Color) : void{
		if (this.IsPolygonOnScreen(vertices, vertexCount)) {
			this.m_sprite.lineStyle(1, color.color, this.m_alpha);
	...
	public DrawSolidPolygon(vertices:Array<any>, vertexCount:number, color:b2Color, isHighlighted:boolean = false, drawOutlines:boolean = true) : void{
		if (this.IsPolygonOnScreen(vertices, vertexCount)) {
			...
			if (drawOutlines) this.m_sprite.lineStyle(this.m_lineThickness * this.m_drawScale, (this.drawColours ? (isHighlighted ? b2DebugDraw.DarkenColour(outlineColour).color : outlineColour.color) : color.color), this.m_alpha);
			else this.m_sprite.lineStyle(0, 0, 0);
			this.m_sprite.moveTo(vertices[0].x * this.m_drawScale - this.m_drawXOff, vertices[0].y * this.m_drawScale - this.m_drawYOff);
			this.m_sprite.beginFill((this.drawColours ? (isHighlighted ? b2DebugDraw.BrightenColour(color).color : color.color) : color.color), this.m_fillAlpha * (isHighlighted ? 0.8 : 1));
			for (var i:number = 1; i < vertexCount; i++){
				this.m_sprite.lineTo(vertices[i].x * this.m_drawScale - this.m_drawXOff, vertices[i].y * this.m_drawScale - this.m_drawYOff);
			}
			this.m_sprite.lineTo(vertices[0].x * this.m_drawScale - this.m_drawXOff, vertices[0].y * this.m_drawScale - this.m_drawYOff);
	...
	public DrawSolidCannon(vertices:Array<any>, vertexCount:number, color:b2Color, isHighlighted:boolean = false, drawOutlines:boolean = true) : void{
		if (this.IsPolygonOnScreen(vertices, vertexCount) ) {
			...
			this.m_sprite.moveTo(vertices[0].x * this.m_drawScale - this.m_drawXOff, vertices[0].y * this.m_drawScale - this.m_drawYOff);
			this.m_sprite.beginFill(color.color, this.m_fillAlpha * (isHighlighted ? 0.8 : 1));
	...
	public DrawSolidSegment(p1:b2Vec2, p2:b2Vec2, color:b2Color) : void{
		this.m_sprite.lineStyle(this.m_lineThickness * this.m_drawScale, color.color, this.m_alpha);
		this.m_sprite.moveTo(p1.x * this.m_drawScale - this.m_drawXOff, p1.y * this.m_drawScale - this.m_drawYOff);
		this.m_sprite.lineTo(p2.x * this.m_drawScale - this.m_drawXOff, p2.y * this.m_drawScale - this.m_drawYOff);
	}
	...
	public DrawXForm(xf:b2XForm) : void{
		this.m_sprite.lineStyle(1, 0xff0000, this.m_alpha);
		this.m_sprite.moveTo(xf.position.x * this.m_drawScale - this.m_drawXOff, xf.position.y * this.m_drawScale - this.m_drawYOff);
		this.m_sprite.lineTo((xf.position.x + this.m_xformScale*xf.R.col1.x) * this.m_drawScale - this.m_drawXOff, (xf.position.y + this.m_xformScale*xf.R.col1.y) * this.m_drawScale - this.m_drawYOff);

		this.m_sprite.lineStyle(1, 0x00ff00, this.m_alpha);
		this.m_sprite.moveTo(xf.position.x * this.m_drawScale - this.m_drawXOff, xf.position.y * this.m_drawScale - this.m_drawYOff);
		this.m_sprite.lineTo((xf.position.x + this.m_xformScale*xf.R.col2.x) * this.m_drawScale - this.m_drawXOff, (xf.position.y + this.m_xformScale*xf.R.col2.y) * this.m_drawScale - this.m_drawYOff);
	}
	...
	public static BrightenColour(color:b2Color):b2Color {
	    ...
	    return new b2Color(newB / 255.0, newG / 255.0, newR / 255.0);
	}
	...
	public static GetR(colour:b2Color):number {
		return (colour.color & 0xFF0000) >> 16;
	}

	public static GetG(colour:b2Color):number {
		return (colour.color & 0x00FF00) >> 8;
	}

	public static GetB(colour:b2Color):number {
		return (colour.color & 0x0000FF);
	}
