package Gui
{
	...
	public class MainEditPanel extends GuiWindow
	{
		...
		public function MainEditPanel(contr:ControllerGame) {
			...
			m_loginButton.visible = (ControllerGameGlobals.userName == "_Public");
			m_logoutButton = new GuiButton("Logout", 610, 45, 85, 35, cont.logoutButton, GuiButton.PURPLE);
			m_logoutButton.visible = (ControllerGameGlobals.userName != "_Public");
			...
			m_resetButton = new GuiButton("Stop", 700, 35, 85, 45, (ControllerGameGlobals.playingReplay ? cont.rewindButton : cont.resetButton), GuiButton.PLAY, format);
			m_gamePanel.addChild(m_resetButton);
			m_resetButton2 = new GuiButton("Stop", 700, 35, 85, 45, (ControllerGameGlobals.playingReplay ? cont.rewindButton : cont.resetButton), GuiButton.PLAY, format);
			m_pausePanel.addChild(m_resetButton2);
			m_rewindButton = new GuiButton((ControllerGameGlobals.playingReplay ? "Rewind" : "Restart"), 535, 45, 75, 35, (ControllerGameGlobals.playingReplay ? cont.resetButton : cont.rewindButton), GuiButton.RED);
			m_gamePanel.addChild(m_rewindButton);
			m_rewindButton2 = new GuiButton((ControllerGameGlobals.playingReplay ? "Rewind" : "Restart"), 535, 45, 75, 35, (ControllerGameGlobals.playingReplay ? cont.resetButton : cont.rewindButton), GuiButton.RED);
			...
			if (ControllerGameGlobals.playingReplay) m_header.text = "    Replay in Progress";
			...
			m_header = new TextField();
			if (ControllerGameGlobals.playingReplay) m_header.text = "    Replay Paused";
			...
			ShowFeatureButton();
			m_rateButton = new GuiButton((ControllerGameGlobals.playingReplay ? "Rate this Replay!" : "Rate this Robot!"), 165, 15, 140, 35, rateButton, GuiButton.ORANGE);
			m_gamePanel.addChild(m_rateButton);
			m_rateButton2 = new GuiButton((ControllerGameGlobals.playingReplay ? "Rate this Replay!" : "Rate this Robot!"), 165, 15, 140, 35, rateButton, GuiButton.ORANGE);
			...
			m_commentButton = new GuiButton((ControllerGameGlobals.playingReplay ? "Comment on this Replay" : "Comment on this Robot"), 15, 45, 160, 35, commentButton, GuiButton.ORANGE);
			m_gamePanel.addChild(m_commentButton);
			m_commentButton2 = new GuiButton((ControllerGameGlobals.playingReplay ? "Comment on this Replay" : "Comment on this Robot"), 15, 45, 160, 35, commentButton, GuiButton.ORANGE);
			...
			m_linkButton = new GuiButton((ControllerGameGlobals.playingReplay ? "Link to this Replay" : "Link to this Robot"), 165, 45, 140, 35, linkButton, GuiButton.ORANGE);
			m_gamePanel.addChild(m_linkButton);
			m_linkButton2 = new GuiButton((ControllerGameGlobals.playingReplay ? "Link to this Replay" : "Link to this Robot"), 165, 45, 140, 35, linkButton, GuiButton.ORANGE);
			...
			m_embedButton = new GuiButton((ControllerGameGlobals.playingReplay ? "Embed this Replay" : "Embed this Robot"), 295, 45, 140, 35, embedButton, GuiButton.ORANGE);
			m_gamePanel.addChild(m_embedButton);
			m_embedButton2 = new GuiButton((ControllerGameGlobals.playingReplay ? "Embed this Replay" : "Embed this Robot"), 295, 45, 140, 35, embedButton, GuiButton.ORANGE);
		...
		public function Update(curAction:int):void {
			if (!Main.inIFrame) {
				if (curAction == ControllerGameGlobals.NEW_CIRCLE) m_circleButton.SetState(true);
				else if (m_circleButton.depressed) m_circleButton.SetState(false);

				if (curAction == ControllerGameGlobals.NEW_RECT) m_rectButton.SetState(true);
				else if (m_rectButton.depressed) m_rectButton.SetState(false);

				if (curAction == ControllerGameGlobals.NEW_TRIANGLE) m_triangleButton.SetState(true);
				else if (m_triangleButton.depressed) m_triangleButton.SetState(false);

				if (curAction == ControllerGameGlobals.NEW_FIXED_JOINT) m_fixedJointButton.SetState(true);
				else if (m_fixedJointButton.depressed) m_fixedJointButton.SetState(false);

				if (curAction == ControllerGameGlobals.NEW_REVOLUTE_JOINT) m_revoluteJointButton.SetState(true);
				else if (m_revoluteJointButton.depressed) m_revoluteJointButton.SetState(false);

				if (curAction == ControllerGameGlobals.NEW_PRISMATIC_JOINT) m_prismaticJointButton.SetState(true);
				else if (m_prismaticJointButton.depressed) m_prismaticJointButton.SetState(false);

				if (curAction == ControllerGameGlobals.NEW_TEXT) m_textButton.SetState(true);
				else if (m_textButton.depressed) m_textButton.SetState(false);
			}
			if (ControllerGameGlobals.playingReplay) {
				m_featureButton.label = (ControllerGameGlobals.curReplayFeatured ? "Un-feature!" : "Feature!");
				m_featureButton2.label = (ControllerGameGlobals.curReplayFeatured ? "Un-feature!" : "Feature!");
			} else if (ControllerGameGlobals.curChallengeID != "") {
				m_featureButton.label = (ControllerGameGlobals.curChallengeFeatured ? "Un-feature!" : "Feature!");
				m_featureButton2.label = (ControllerGameGlobals.curChallengeFeatured ? "Un-feature!" : "Feature!");
			} else {
				m_featureButton.label = (ControllerGameGlobals.curRobotFeatured ? "Un-feature!" : "Feature!");
				m_featureButton2.label = (ControllerGameGlobals.curRobotFeatured ? "Un-feature!" : "Feature!");
			}
		}
		...
		public function ShowFeatureButton():void {
			m_featureButton.visible = Util.ObjectInArray(ControllerGameGlobals.userName, ControllerGameGlobals.ADMIN_USERS);
			m_featureButton2.visible = Util.ObjectInArray(ControllerGameGlobals.userName, ControllerGameGlobals.ADMIN_USERS);
		}
